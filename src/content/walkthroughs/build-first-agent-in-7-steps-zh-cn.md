...
---> [正体中文](/walkthroughs/build-first-agent-in-7-steps/) | **简体中文** | [English](/walkthroughs/build-first-agent-in-7-steps-en/)
...
> 走 [Track A（CLI Power User）](/tracks/a1-cli-intro-zh-cn/) 的人**不需要跑**这份；但读过之后对「**agent 从 LLM API 到 production 怎么一步步组起来**」会有更深的理解，可作为 optional 进阶补充。
...
**进阶**：如果想把这个 skill 包成可分享的 plugin（让别人也能装在自己的 Claude Code），参考 [Stage 5.4 Plugins & Marketplaces](/stages/05-claude-code-ecosystem-zh-cn/#54--plugins-与-marketplaces)。本 walkthrough 不展开 plugin 打包流程。
