...
description: "This is a concrete cross-stage walkthrough — the same agent, traced from Stage 1 through Stage 7, with executable code skeletons at each stage; after that, use Stage 8 to choose the smallest, safest interface."
section: "Walkthrough"
...
- ⭐⭐⭐⭐⭐ [Stage 8 — Agent Interfaces](/en/stages/08-agent-interfaces/): start with API/Fetch and upgrade to Browser, Computer, or Sandbox only when needed.
...
Run the [Stage 7 Safe Execution example](/en) directly; it uses no network or model and tests crashes, late rejection, ledger conflicts, and at-most-once execution.
...
1. Read [Stage 7.5 — Advanced Agentic Concepts](/en/stages/07-5-advanced-agentic-concepts/) and choose only the advanced ideas this system actually needs.
2. Then read the full [Stage 8 — Agent Interfaces](/en/stages/08-agent-interfaces/) to confirm the current API/Fetch is small enough; upgrade to Browser Use, Computer Use, or Sandbox only when the task truly needs it.
3. To choose a different route, return to the [main-path README](/en).
...
- **Multi-agent paper review**: two agents play supportive vs adversarial reviewer, while a third plays area chair → [researcher path](/en/branches/for-researcher/)
- **Conference report generator**: given a conference proceedings URL, produce per-track high-level summaries → [knowledge-worker path](/en/branches/for-knowledge-worker/)
- **Topic trend tracker**: scan arXiv weekly, compare new papers with existing Memory, and produce a weekly digest → [everyday-user path](/en/branches/for-everyday-users/)
