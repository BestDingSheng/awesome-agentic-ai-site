...
---> **繁體中文** | [English](/walkthroughs/build-first-agent-in-7-steps-en/)
...
> 走 [Track A（CLI Power User）](/tracks/a1-cli-intro/) 的人**不需要跑**這份；但讀過之後對「**agent 從 LLM API 到 production 怎麼一步步組起來**」會有更深的理解，可作為 optional 進階補充。
...
**進階**：如果想把這個 skill 包成可分享的 plugin（讓別人也能裝在自己的 Claude Code），參考 [Stage 5.4 Plugins & Marketplaces](/stages/05-claude-code-ecosystem/#54--plugins-與-marketplaces)。本 walkthrough 不展開 plugin 打包流程。
