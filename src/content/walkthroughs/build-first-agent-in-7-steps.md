...
description: "這是一份跨 7 個 stage 的具體 walkthrough——同一個 agent，從 Stage 1 寫到 Stage 7，每個 stage 都附可執行的程式碼骨架；完成後再用 Stage 8 選最小、最安全的操作介面。"
section: "Walkthrough"
...
---
> **繁體中文** | [简体中文](/walkthroughs/build-first-agent-in-7-steps/) | [English](/en/walkthroughs/build-first-agent-in-7-steps/)
...
> 走 [Track A（CLI Power User）](/zh-tw/tracks/cli-a1-cli-intro/) 的人**不需要跑**這份；但讀過之後對「**agent 從 LLM API 到 production 怎麼一步步組起來**」會有更深的理解，可作為 optional 進階補充。
...
- ⭐⭐⭐⭐⭐ [Stage 8 — Agent 操作介面](/zh-tw/stages/08-agent-interfaces/)：學會先用 API／Fetch，真的需要時才升級到 Browser、Computer 或 Sandbox。
...
**進階**：如果想把這個 skill 包成可分享的 plugin（讓別人也能裝在自己的 Claude Code），參考 [Stage 5.4 Plugins & Marketplaces](/zh-tw/stages/05-claude-code-ecosystem/#54--plugins-與-marketplaces)。本 walkthrough 不展開 plugin 打包流程。
...
直接跑 [Stage 7 Safe Execution 範例](/zh-tw)；它不連網、不用模型，會把 crash、late reject、ledger 衝突與最多執行一次都測給你看。
...
1. 讀 [Stage 7.5 — 進階 Agentic 概念](/zh-tw/stages/07-5-advanced-agentic-concepts/)，替剛完成的系統選真正需要的進階做法。
2. 再讀完整的 [Stage 8 — Agent Interfaces](/zh-tw/stages/08-agent-interfaces/)，確認目前的 API／Fetch 已經夠小；只有任務真的需要時才升級到 Browser Use、Computer Use 或 Sandbox。
3. 想改走另一條路時，回到[主路線 README](/zh-tw)。
...
- **Multi-agent paper review**：兩個 agent 分別當 supportive reviewer 跟 adversarial reviewer，第三個 agent 當 area chair → [研究人員路徑](/zh-tw/branches/for-researcher/)
- **Conference report generator**：給定一個 conference proceedings URL，產出每個 track 的高層摘要 → [知識工作者路徑](/zh-tw/branches/for-knowledge-worker/)
- **同主題論文趨勢追蹤**：每週掃 arXiv，找新論文跟現有 Memory 比較，產出 weekly digest → [日常使用者路徑](/zh-tw/branches/for-everyday-users/)
