...
description: "这一关先检查你是否会用后面一定会用到的四种工具。会的话就直接跳过。不会也没关系。照着下面的小练习做一次即可。"
section: "Stage"
...
> [繁體中文](/zh-tw/stages/00-foundations/) | [English](/en/stages/00-foundations/) | **简体中文**
...
四项都能做到，就直接前往 [Stage 1 — LLM 基础](/stages/01-llm-basics/)。只要有一项不确定，就完成本页的主要练习。需要时再展开补充内容。
...
四项都完成后，就可以前往 [Stage 1 — LLM 基础](/stages/01-llm-basics/)。如果卡住，展开下面最接近问题的部分。不必一次读完。
...
看到其他不懂的词，先查[术语表](/resources/glossary/)。想知道 agent 为什么可能出现在终端、聊天软件或设备上，再看[Agent 全景地图](/resources/agent-paradigms/)。这两份都不是开始主要练习前的必读内容。
...
> ✅ **完成 Stage 0 了吗？** 接着前往 [**Stage 1 — LLM 基础**](/stages/01-llm-basics/)，完成第一次 LLM API 调用，并学习 token、context window 与成本估算。
