---
title: "Stage 4 — Workflow Graph 與 Agent 框架"
description: "你在 Stage 3 已經自己寫過 Agent Loop。這一關先把多步工作畫成 Workflow Graph，再選 Framework（框架） 來幫你接線。先看懂工作地圖，再選工具箱，才不會因為某個框架很流行就硬把事情變複雜。"
section: "Stage"
...
![Agent 系統選擇圖：先分辨由程式或 Agent 決定下一步，再看需要一個或多個 Agent，最後先選最簡單能完成任務的形狀](/upstream/resources/diagrams/agent-framework-choice-map.png)
...
- Claude Code subagent 是 runtime 內建的另一條路：用設定檔隔離 context 與工具，不必自己寫 Python orchestration。完整比較留在 [Stage 5.5](/zh-tw/stages/05-claude-code-ecosystem/#55--subagentsclaude-code-原生-multi-agent-機制-2025-新功能)。
...
都做到後，進入 [Stage 5 — Claude Code Ecosystem](/zh-tw/stages/05-claude-code-ecosystem/)。如果還分不清四格，回到上面的選擇地圖；不必重讀 19 筆表格。
