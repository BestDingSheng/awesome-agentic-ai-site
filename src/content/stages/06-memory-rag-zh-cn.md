---
title: "Stage 6 — 上下文管理（Context Engineering）：RAG 与 Memory"
description: "本 stage 的核心不是“多背一点术语”，而是理解 agent 如何管理 context。"
section: "Stage"
...
> 💡 这个 stage 用语密度高（**RAG / 向量数据库 / embedding / chunking / hybrid search / reranking...**）→ 不熟先翻 [`resources/glossary.zh-Hans.md` 3](/resources/glossary/#3-memory--retrieval--rag)。
>
> 📋 **本章组成**：定位 → 入口 → **RAG 主轴**（基础 + 进阶 + DSPy + Eval）→ **Bridge** → **Memory 主轴**（3 pattern + trio + 进阶）→ Chunking → Reflexion / Reasoning → 练习 → Projects
>
> 🔑 **关键术语**：见 [`resources/glossary.zh-Hans.md` 3](/resources/glossary/#3-memory--retrieval--rag)（memory / RAG / embedding / chunking / reranking）
...
详细对照表见 [Stage 7 五层工程分工](/stages/07-multi-agent-production/#五层工程分工prompt--context--harness--loop--graph)（分层的 canonical 出处）。
...
- 走过 Stage 4（agent frameworks）+ Stage 5（Claude Code 生态）—— curriculum 主线是 **3 → 4 → 5 → 6**（见 [README 学习地图](/#-学习地图两条学习路径)）；非硬性技术前置，但 RAG / memory 常跟 framework + Claude Code memory 机制搭配、照顺序走过理解更完整，且 [Stage 7](/stages/07-multi-agent-production/) 预期你已完成 4 + 5 + 6
- 能够运行 Python `pip install` 来安装 SDK（后续练习会用到 `chromadb`、`sentence-transformers` 等）
- 熟悉 list / dict / generator 等基础 Python 结构

如果没有达到，请回看 [Stage 3](/stages/03-tool-use-and-hello-agent/) 或 [Stage 0 环境设置](/stages/00-foundations/#何时可以跳过这个阶段)。
...
![RAG Pipeline Overview](/upstream/resources/diagrams/rag-pipeline-overview.zh-Hans.png)
...
| **Procedural memory** | 如何执行动作 / 技能 | tool definitions、[Skills（Stage 5.3）](/stages/05-claude-code-ecosystem/#53--skillsclaude-code-的行为层-claude-code-生态最关键的一层) |
...
![Chunking Strategy Flowchart](/upstream/resources/diagrams/chunking-strategies.zh-Hans.png)
...
> - 想看 production agent 内部如何使用反思（例如 Cursor / Claude Code）→ [Stage 5 5.7 Harness Internals](/stages/05-claude-code-ecosystem/#57--claude-code-source-解剖reference-harness-implementation-track-b-必看)。
...
| Multi-agent debate / critic 场景 | Path 1（CRITIC / debate）+ [Stage 7 Multi-agent](/stages/07-multi-agent-production/) |
...
|---|---|---|---|---|
| **RAG Framework**<br>（完整流水线） | [LlamaIndex](https://github.com/run-llama/llama_index) | ⭐⭐⭐⭐⭐ | 以文档为核心的应用 | 以 RAG 为核心，提供 document loader / chunking / retrieval / query engine 一站式服务。★ 51k+ |
...
如果以上都能够做到 → 前往 [Stage 7 — Multi-Agent · Production 化](/stages/07-multi-agent-production/)。
