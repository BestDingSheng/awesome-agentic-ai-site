...
![RAG 检索外部证据；Memory 写入并读回重要状态](/upstream/resources/diagrams/rag-memory-map.zh-Hans.png)
...
[打开完整说明与检查方式](/)。不要先背一个“标准大小”；先看文件结构与测试结果。
...
[打开完整说明与检查方式](/)。只保存完成任务需要的数据，并提供查看、修改与删除的方法。
...
![RAG 先整理数据；问题来时再取回候选、整理证据并附来源回答](/upstream/resources/diagrams/rag-pipeline-overview.zh-Hans.png)
