...
description: "Prompt（提示）不只是一个问题。它是交给模型的一整份任务包，可以放进指令、要处理的资料、范例和输出规则。"
section: "Stage"
...
- **先备**：完成 [Stage 1](/stages/01-llm-basics/)，并能运行一段 Python。
...
标签可以帮助整理内容，但不是完整的安全墙。正式的 prompt injection 防护放在 [Stage 8](/stages/08-agent-interfaces/)。

### 选修 3：需要严格 JSON

只写“请返回 JSON”不能保证每次都合法。程序必须在解析失败时明确报错。需要固定 schema 时，改用 [Stage 3](/stages/03-tool-use-and-hello-agent/) 的 Structured Outputs 或 tool schema。
...
| Context Engineering | 这一次把哪些资料放进 context window | [Stage 6](/stages/06-memory-rag/) |
| Harness Engineering | 模型外面的 loop、retry、sandbox、eval 和 observability | [Stage 7](/stages/07-multi-agent-production/) |
