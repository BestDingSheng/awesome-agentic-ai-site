---
title: "Stage 0 — 基礎準備（Foundations）"
description: "這一關先檢查：你會不會使用後面一定會用到的四種工具？會就直接跳過。不會也沒關係，照著下面的小練習做一次。"
section: "Stage"
...
四項都做得到，就直接前往 [Stage 1 — LLM 基礎](/zh-tw/stages/01-llm-basics/)。只要有一項不確定，就完成本頁的主練習；需要時再展開補充內容。
...
四項都完成，就可以前往 [Stage 1 — LLM 基礎](/zh-tw/stages/01-llm-basics/)。如果卡住，展開下面最接近問題的部分，不必一次讀完。
...
看到其他不懂的詞，先查 [術語表](/zh-tw/resources/glossary/)。想知道 agent 為什麼可能出現在終端機、聊天軟體或裝置上，再看 [Agent 全景地圖](/zh-tw/resources/agent-paradigms/)。這兩份都不是開始主練習前的必讀內容。
...
> ✅ **走完 Stage 0 了？** 接著前往 [**Stage 1 — LLM 基礎**](/zh-tw/stages/01-llm-basics/)，完成第一次 LLM API 呼叫，並學會 token、context window 與成本估算。
