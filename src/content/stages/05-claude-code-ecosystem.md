...
description: "| Layer | 是什麼 | Claude 的版本 | 誰管 | 學在 | |---|---|---|---|---| | L7 Interface | 使用者跟 agent 交談入口 | claude-code CLI / Desktop | Harness Eng | Stage 5.1 | | L6 Workflow | 固定可重用流程模板 | Skills（SKILL.md）+ Slash commands + Plugins"
section: "Stage"
...
> 🚪 **進入條件與兩條軌**（本 stage 跟 [Stage 8 — Agent Interfaces](/zh-tw/stages/08-agent-interfaces/) 是 curriculum 的兩個共用 hub）：
...
> - **Track B（Agent Builder）**：建議先完成 [Stage 3](/zh-tw/stages/03-tool-use-and-hello-agent/)（tool use）+ [Stage 4](/zh-tw/stages/04-agent-frameworks/)（agent frameworks）再進，把整個 stage 當「**Claude Code 內部怎麼運作**」深讀，從 5.1 完整走到 5.7

> 📋 **本章組成**：7 個核心子章（5.1 基礎 / 5.2 MCP / 5.3 Skills / 5.4 Plugins / 5.5 Subagents / 5.6 Dynamic Workflows / 5.7 Claude Code Source 解剖）+ 5.8 SDK（選修、包成產品或服務才需要）。每個子章都是「學習目標 → 必修閱讀 → 動手練習 → 精選 Projects」，章末有自我檢查。整個 stage 圍繞 4 個關鍵詞：**MCP / Skills / Plugins / Marketplace**——不熟先翻 [`resources/glossary.md` 5](/zh-tw/resources/glossary/#5-claude-code-生態)。
...
Stage 5 接下來專講 Claude Code。想看完整 5 種 agent paradigm 對照 → [`resources/agent-paradigms.md`](/zh-tw/resources/agent-paradigms/)（Claude Code 是 Type 1 IDE-coupled + Type 2 Terminal pair-programmer）。
