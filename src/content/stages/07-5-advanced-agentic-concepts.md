...
description: "這一關不是要你背完所有新名詞。它像一張地圖：你的 Agent 卡在哪裡，就先看那一小塊。"
section: "Stage"
...
完成 [Stage 7](/zh-tw/stages/07-multi-agent-production/) 就可以開始。若你跳著讀，至少要做過一個有工具、Eval 與停止條件的 Agent。
...
- **先備觀念**：Agent、Workflow、Handoff、Eval、Observability、Guardrail。忘記時回 [Stage 7 核心詞](/zh-tw/stages/07-multi-agent-production/)。
...
![12 個進階 Agentic 概念：問題分組地圖](/upstream/resources/diagrams/concept-cluster.png)
...
![卡在哪裡，就先讀哪一組](/upstream/resources/diagrams/reading-decision-tree.png)
...
| 邊界 | 為什麼需要 | 對照章節 |
|---|---|---|
| Repo 狀態與 diff | 要知道改了什麼，也要能回復。 | [Stage 5](/zh-tw/stages/05-claude-code-ecosystem/) |
| 隔離的執行環境 | 新程式要真的跑，但不能污染 host。 | [Stage 8](/zh-tw/stages/08-agent-interfaces/) |
| 長任務的 artifact 交接 | 新 session 要知道上一段做到哪裡。 | [Stage 7](/zh-tw/stages/07-multi-agent-production/) |
...
![Model–Harness Fit：用同一組 Eval 決定保留、簡化或移除](/upstream/resources/diagrams/model-harness-fit.png)
...
五項都能做到，就進入 [Stage 8 — Agent 操作介面](/zh-tw/stages/08-agent-interfaces/)：學 Agent 怎麼安全操作 Browser、Computer 與 Code Sandbox。
