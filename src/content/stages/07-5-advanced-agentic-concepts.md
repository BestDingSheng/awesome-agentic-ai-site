---
title: "Stage 7.5 — 進階 Agentic 選擇：有證據才增加複雜度"
description: "← 先回到 Stage 7 — Agent 上線工程：若你還不能用 Eval、Trace、Approval 與 Recovery 證明系統可測、可看、可停、可恢復，先不要增加本章的進階做法。"
section: "Stage"
...
← **先回到 [Stage 7 — Agent 上線工程](/zh-tw/stages/07-multi-agent-production/)**：若你還不能用 Eval、Trace、Approval 與 Recovery 證明系統可測、可看、可停、可恢復，先不要增加本章的進階做法。
...
完成 [Stage 7](/zh-tw/stages/07-multi-agent-production/)，並已經有一組能重跑的 Eval cases、停止條件與單一 Agent baseline。這一關不要求寫程式，也不要求購買 API 或公開模型的私人 Chain-of-Thought。
...
![進階 Agentic 選擇：從失敗證據走到最小必要做法](/upstream/resources/diagrams/advanced-agentic-decision-map.png)
...
五項都能做到，就進入 [Stage 8 — Agent 操作介面](/zh-tw/stages/08-agent-interfaces/)。如果還沒有 baseline 或失敗證據，先回到 [Stage 7](/zh-tw/stages/07-multi-agent-production/) 補齊，而不是再加一個 Agent。
