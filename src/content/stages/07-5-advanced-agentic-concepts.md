...
- **OpenAI Agents SDK 的 Sandbox Agents** 把 workspace、session、approval 與 sandbox client 接進同一套 Agent API；[Stage 4 已標註](/zh-tw/stages/04-agent-frameworks/)這仍是 beta，不能因此宣稱所有 production 問題都已解決。比較值得觀察的是：harness 的共用抽象正在成形。
