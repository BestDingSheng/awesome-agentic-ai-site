---
title: "Stage 7.5 — 進階 Agentic 概念地圖（Advanced Agentic Concepts Map）"
description: "⏱ 時間估算：1 週（約 5 小時——不寫 code、只讀資源建立概念地圖）"
section: "Stage"
...
> **繁體中文** | [简体中文](/stages/07-5-advanced-agentic-concepts/) | [English](/en/stages/07-5-advanced-agentic-concepts/)

⏱ **時間估算**：1 週（約 5 小時——不寫 code、只讀資源建立概念地圖）

> 🚪 **進入條件**：完成 [Stage 7 — 多 Agent 系統與穩定運作](/zh-tw/stages/07-multi-agent-production/)（或至少 Stage 4 + 6 + 7）。本章是 production 之後的 frontier 概念地圖、不是入門——沒做過 production agent 會讀不出這些概念在解什麼痛點。
...
![Agentic Stack 四層工作邊界](/upstream/resources/diagrams/stack-4layer.png)
...
![5 個 Harness Engineering 原則之間的 cross-source dependency](/upstream/resources/diagrams/principle-dependency.png)
...
下表展示 5 個原則怎麼作用到 [Stage 7 Harness Engineering 的 8 個核心元件](/zh-tw/stages/07-multi-agent-production/#harness-的-8-個核心元件)上（✓ = applies、✓★ = primary lever）：
...
- **OpenAI Agents SDK 2026-04 更新**把 sandbox（7 個 provider）+ harness 抽象層內建——[Stage 4 已標註](/zh-tw/stages/04-agent-frameworks/)這是「production coding agent 首次 architecturally sound」。harness 不再是各家自己拼、開始有共用抽象。
...
> 📚 **想深入 eval rigor**：[Establishing Best Practices for Building Rigorous Agentic Benchmarks](https://arxiv.org/abs/2507.02825) 是系統性的盤點；reward hacking 的 production 警告見 [Stage 7 Benchmark Landscape](/zh-tw/stages/07-multi-agent-production/) 的 Berkeley 段 + τ-bench（[sierra-research/tau2-bench](https://github.com/sierra-research/tau2-bench)）的 pass^k 設計。

### 🔀 Dynamic Workflows（Opus 4.8）— 當 agent 自己寫出 workflow

上面 coding-agent harness 提到 Opus 4.8 的 **Dynamic Workflows**（2026-05-28、Claude Code research preview）。它值得單獨拆開——因為它把 [Stage 4 教的 workflow vs agent 區分](/zh-tw/stages/04-agent-frameworks/#兩個維度先分清楚workflow-vs-agent--single-vs-multi)整個塌縮掉，是理解「agent-authored orchestration」最好的活教材。

> 📌 **Claude Fable 5（2026-06-09 發布、2026-06-12 暫停存取）**：Anthropic 的 Mythos-class **Claude Fable 5**（`claude-fable-5`、定位在 Opus 之上）曾短暫成為廣泛可用的最高能力成員，與 limited-availability 的 Claude Mythos 5（`claude-mythos-5`）同日發布。⚠️ **2026-06-12 美國出口管制指令暫停了兩者全部存取（[狀態頁](https://status.claude.com/) · [官方聲明](https://www.anthropic.com/news/fable-mythos-access)），目前無法使用且無恢復時程。** Opus 4.8 仍是 Opus-class 旗艦、也是目前可用的最高層級，**Dynamic Workflows 仍是 Opus 4.8 的功能**，不歸屬於 Fable 5。
...
> 🛠️ **想直接動手寫 SKILL.md / CLAUDE.md？** 4 個實作用 prompt（audit 既有 / 生成新的）已**移到 [Stage 5](/zh-tw/stages/05-claude-code-ecosystem/)**——讀者真正動手寫的地方：
> - [Stage 5.1  CLAUDE.md 設計 prompts](/zh-tw/stages/05-claude-code-ecosystem/#-claudemd-設計-prompts依-5-原則)
> - [Stage 5.3  SKILL.md 設計 prompts](/zh-tw/stages/05-claude-code-ecosystem/#-skillmd-設計-prompts含-skill-creator-替代)
...
→ **建議用法**：學完上面 5 個原則後跑這個 quiz 確認你抓到 concept；實作面（寫 SKILL.md / CLAUDE.md）的 prompt 在 [Stage 5](/zh-tw/stages/05-claude-code-ecosystem/)。
...
4. **Encode 到 Skills + CLAUDE.md**：用 Stage 5 的 4 個 prompt 動手寫——CLAUDE.md audit / generate（[Stage 5.1](/zh-tw/stages/05-claude-code-ecosystem/#51--claude-code-基礎)）+ SKILL.md audit / generate（[Stage 5.3](/zh-tw/stages/05-claude-code-ecosystem/#53--skillsclaude-code-的行為層-claude-code-生態最關鍵的一層)）。
...
> 📋 **學完進階概念回頭看 synthesis** → [Stage 5 §🗺️ 7-Layer Architecture Map](/zh-tw/stages/05-claude-code-ecosystem/#-7-layer-architecture-map先看這張圖再讀-51-57)（Claude Code 7 個 primitive + 3 個工程學 discipline 整合圖）
...
→ 接下來 [**Stage 8 — Agent 操作介面（Agent Interfaces）**](/zh-tw/stages/08-agent-interfaces/)（**兩 track 共用 hub**）：學 agent 怎麼跟非 API 世界互動（Computer Use / Browser Use / Code Sandbox）。或挑一個[特化分支](/zh-tw#️-學習地圖兩條學習路徑)、或回過頭來貢獻這份 repo。
