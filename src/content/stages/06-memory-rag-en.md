...
description: "Models do not know everything. RAG is like asking a model to consult a book before it answers; Memory is like giving it a notebook for things it will need next time. This stage distinguishes the two, then helps you build"
section: "Stage"
...
![RAG retrieves external evidence; Memory writes and reads back important state](/upstream/resources/diagrams/rag-memory-map.en.png)
...
[Open the full instructions and checks](/en). Start with very few sentences to avoid unnecessary API costs.
...
[Open the full instructions and checks](/en). Practice data must not include secrets or personal data.
...
[Open the full instructions and checks](/en). Do not memorize a “standard size” first; inspect document structure and test results.
...
[Open the full instructions and checks](/en). Start with a small dataset; do not confuse “the code runs” with “the answer is correct.”
...
[Open the full instructions and checks](/en). Store only what the task requires, and provide ways to view, edit, and delete it.
...
![RAG prepares data first; when a question arrives, it retrieves candidates, organizes evidence, and answers with citations](/upstream/resources/diagrams/rag-pipeline-overview.en.png)
