...
### 精選 Projects
- 跑完 動手練習 後的延伸學習
- 每個 entry 照 [style guide](/resources/style-guide/) §1 schema
...
## Entry 選入 / 排除原則（補強 [style-guide](/resources/style-guide/)）
...
[`walkthroughs/build-first-agent-in-7-steps.md`](/walkthroughs/build-first-agent-in-7-steps/) 用同一個 Paper Summary Bot 串完 Stage 1 到 7。這份是 stage 之間銜接的 ground truth：每個 stage 結束時 agent 應該長什麼樣，下一 stage 怎麼增加新層。
...
- **branch 設計理由**：見 [`../branches/DESIGN.md`](/branches/design/)
- **entry schema / 用詞規範**：見 [`../resources/style-guide.md`](/resources/style-guide/)
- **跨 stage 範例**：見 [`../walkthroughs/build-first-agent-in-7-steps.md`](/walkthroughs/build-first-agent-in-7-steps/)
