---
title: "Stage 7.5 — 进阶 Agentic 选择：有证据才增加复杂度"
description: "← 先回到 Stage 7 — Agent 上线工程：若你还不能用 Eval、Trace、Approval 与 Recovery 证明系统可测、可看、可停、可恢复，先不要增加本章的进阶做法。"
section: "Stage"
...
← **先回到 [Stage 7 — Agent 上线工程](/stages/07-multi-agent-production/)**：若你还不能用 Eval、Trace、Approval 与 Recovery 证明系统可测、可看、可停、可恢复，先不要增加本章的进阶做法。
...
完成 [Stage 7](/stages/07-multi-agent-production/)，并已经有一组能重跑的 Eval cases、停止条件与单一 Agent baseline。这一关不要求写程序，也不要求购买 API 或公开模型的私人 Chain-of-Thought。
...
![进阶 Agentic 选择：从失败证据走到最小必要做法](/upstream/resources/diagrams/advanced-agentic-decision-map.zh-Hans.png)
...
五项都能做到，就进入 [Stage 8 — Agent 操作介面](/stages/08-agent-interfaces/)。如果还没有 baseline 或失败证据，先回到 [Stage 7](/stages/07-multi-agent-production/) 补齐，而不是再加一个 Agent。
