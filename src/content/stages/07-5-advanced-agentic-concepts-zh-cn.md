...
**历史注记**：Dynamic Workflows 于 2026-05-28 与 Claude Opus 4.8 一起发布；这是发布背景，不代表功能绑定某个 Claude model。现在，Dynamic Workflows 是 Claude Code 功能，需要 Claude Code v2.1.154+，不绑定特定 Claude model；完整的 provider 可用范围、Pro 启用方式与监看方式见下表。它值得单独拆开——因为它把 [Stage 4 教的 workflow vs agent 区分](/stages/04-agent-frameworks/#两个维度先分清楚workflow-vs-agent--single-vs-multi)整个塌缩掉，是理解"agent-authored orchestration"最好的活教材。
