...
description: "这一关不是要你背完所有新名词。它像一张地图：你的 Agent 卡在哪里，就先看那一小块。"
section: "Stage"
...
完成 [Stage 7](/stages/07-multi-agent-production/) 就可以开始。若你跳着读，至少要做过一个有工具、Eval 与停止条件的 Agent。
...
- **先备观念**：Agent、Workflow、Handoff、Eval、Observability、Guardrail。忘记时回 [Stage 7 核心词](/stages/07-multi-agent-production/)。
...
![12 个进阶 Agentic 概念：问题分组地图](/upstream/resources/diagrams/concept-cluster.zh-Hans.png)
...
![卡在哪里，就先读哪一组](/upstream/resources/diagrams/reading-decision-tree.zh-Hans.png)
...
| 边界 | 为什么需要 | 对照章节 |
|---|---|---|
| Repo 状态与 diff | 要知道改了什么，也要能恢复。 | [Stage 5](/stages/05-claude-code-ecosystem/) |
| 隔离的执行环境 | 新程序要真的跑，但不能污染 host。 | [Stage 8](/stages/08-agent-interfaces/) |
| 长任务的 artifact 交接 | 新 session 要知道上一段做到哪里。 | [Stage 7](/stages/07-multi-agent-production/) |
...
![Model–Harness Fit：用同一组 Eval 决定保留、简化或移除](/upstream/resources/diagrams/model-harness-fit.zh-Hans.png)
...
五项都能做到，就进入 [Stage 8 — Agent 操作界面](/stages/08-agent-interfaces/)：学 Agent 怎么安全操作 Browser、Computer 与 Code Sandbox。
