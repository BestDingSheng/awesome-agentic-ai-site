...
你能运行一个 Python 文件、看懂 function 与 dict，并完成 [Stage 02](/stages/02-prompt-engineering/)，就可以开始。环境还没准备好时，先回 [Stage 00](/stages/00-foundations/)。
...
![Tool Use 六步图：模型提出 Tool Call，程序验证并执行，再把 Tool Result 送回模型。](/upstream/resources/diagrams/tool-use-loop.zh-Hans.png)
...
| 名称 | 白话用途 | 在哪里学 |
|---|---|---|
| **Chain-of-Thought（CoT）** | 早期 prompt 技巧常要求写出中间推理。现在不把完整私有思维链当成通用输出要求；需要检查时，看最终答案与简短、可验证的理由 | [Stage 02](/stages/02-prompt-engineering/) |
| **ReAct** | 在循环中交替采取 action、读 observation，再决定下一步 | 本章练习 3 |
| **Reflection** | 用一轮反馈改进下一次尝试的广义做法 | 本章下方路由 |
| **Reflexion／Self-Refine** | 有明确 Actor／Critic 或自我反馈流程的研究 pattern | 本章概念；持久记忆版见 [Stage 06](/stages/06-memory-rag/) |
| **Planning** | 先拆成多步，再根据结果调整计划 | [Stage 07.5](/stages/07-5-advanced-agentic-concepts/) |
...
- [Path A README（Ollama）](/)：运行 `python starter.py`。
...
完整双路径在 [`06-schema-design`](/)，速查表在 [`resources/schema-design-cheatsheet.zh-Hans.md`](/resources/schema-design-cheatsheet/)。
...
本章只理解 single-session loop。需要跨 session 保存失败经验时，进入 [Stage 06 的 Reflection Memory](/stages/06-memory-rag/)；需要更完整的 planning、verification 与长时间执行时，进入 [Stage 07.5](/stages/07-5-advanced-agentic-concepts/)。
