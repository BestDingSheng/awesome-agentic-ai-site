---
title: "Stage 6 — 上下文管理（Context Engineering）：RAG 與 Memory"
description: "本 stage 的核心不是「多背一點名詞」，而是理解 agent 如何管理 context。"
section: "Stage"
...
- 走過 Stage 4（agent frameworks）+ Stage 5（Claude Code 生態）—— curriculum 主線是 **3 → 4 → 5 → 6**（見 [README 學習地圖](/zh-tw#️-學習地圖兩條學習路徑)）；非硬性技術前置，但 RAG / memory 常跟 framework + Claude Code memory 機制搭配、照順序走過理解更完整，且 [Stage 7](/zh-tw/stages/07-multi-agent-production/) 預期你已完成 4 + 5 + 6
- 能跑 Python `pip install` 安裝 SDK（後面練習會用到 `chromadb`、`sentence-transformers` 等）
- 對 list / dict / generator 等基礎 Python 結構上手

沒到的話 → 回 [Stage 3](/zh-tw/stages/03-tool-use-and-hello-agent/) 或 [Stage 0 環境設定](/zh-tw/stages/00-foundations/#何時可以跳過這個階段)。
...
![RAG 流水線總覽](/upstream/resources/diagrams/rag-pipeline-overview.jpg)
...
| **Procedural memory** | 怎麼做事的程式 / skill | tool definitions、[Skills（Stage 5.3）](/zh-tw/stages/05-claude-code-ecosystem/#53--skillsclaude-code-的行為層-claude-code-生態最關鍵的一層) |
...
![Chunking 策略流程](/upstream/resources/diagrams/chunking-strategies.jpg)
...
> **本節是 concept + routing、不是練習**。延續 [Stage 3 反思](/zh-tw/stages/03-tool-use-and-hello-agent/#-反思reflexion--self-refine-概念--路由) 的基本版（single-session Actor / Critic loop），講為什麼有些反思**需要**持久記憶——這版本才真正屬於 Stage 6 主題。
...
| Model | 來源 / 發布 | 特色 | 連結 |
|---|---|---|---|
| **GPT-5.5** | OpenAI 2026-04（前身：o1 2024-09 → o3 → GPT-5 2025-08 → 5.4 2026-03）| 閉源、reasoning + chat 合併、Thinking budget API、agent 能力強化。**較新層級：GPT-5.6（Sol / Terra / Luna）、2026-06 限量 preview** | [OpenAI](https://openai.com/) |
| **Claude Fable 5** | Anthropic 2026-06（Mythos-class、位階在 Opus class 之上；同步發布 Claude Mythos 5 為解除部分 safeguard 的限量版本）| 閉源、Mythos-class（位階在 Opus class 之上）。⚠️ **存取已於 2026-06-12 被美國出口管制指令暫停（[狀態頁](https://status.claude.com/)）；Fable 5 與 Mythos 5 目前皆無法使用、無恢復時程、請改用 Opus 4.8。** 官方 benchmark 數字始終未公布 | [Claude Fable 5 / Mythos 5](https://www.anthropic.com/news/claude-fable-5-mythos-5) |
| **Claude Opus 4.8** | Anthropic 2026-05（前身：Sonnet 4.5 / Opus 4.5 / Opus 4.7、Dynamic Workflows 研究預覽）| 閉源、Opus class 旗艦、目前可用的最高 Claude 層級（原為 Fable 5 的 safeguard fallback；Fable 5 已於 2026-06-12 暫停）、可控 thinking budget（API 參數）、**SWE-bench / Terminal-bench 領先** | [Anthropic extended thinking](https://docs.claude.com/en/docs/build-with-claude/extended-thinking) |
...
| **Vector DB**<br>（跑在 app 內） | [lancedb/lancedb](https://github.com/lancedb/lancedb) | ⭐⭐⭐⭐ | 想要 vector DB 直接內建、不想另跑 server | 直接跑在你 app 裡的 vector DB（不用另開 server）、能處理文字 + 圖片、關鍵字 + 向量一起搜。★ 10k+、Apache-2.0 |
...
如果都可以 → 前往 [Stage 7 — Multi-Agent · 進階應用](/zh-tw/stages/07-multi-agent-production/)。
