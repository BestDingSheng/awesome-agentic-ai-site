...
![RAG 取回外部證據；Memory 寫入並讀回重要狀態](/upstream/resources/diagrams/rag-memory-map.png)
...
| 想穩定改變模型的行為或特定能力 | [**Fine-tuning**](/zh-tw/resources/model-training-guide/) | 調整模型權重與行為；它不會自動替你提供最新文件。 |
...
[打開完整說明與檢查方式](/zh-tw)。先用很少的句子，避免不必要的 API 費用。
...
[打開完整說明與檢查方式](/zh-tw)。練習資料不得包含秘密或個資。
...
[打開完整說明與檢查方式](/zh-tw)。不要先背一個「標準大小」；先看文件結構與測試結果。
...
[打開完整說明與檢查方式](/zh-tw)。先用小型資料集；不要把「程式能跑」當成「回答一定正確」。
...
[打開完整說明與檢查方式](/zh-tw)。只保存完成任務需要的資料，並提供查看、修改與刪除的方法。
...
![RAG 先整理資料；問題來時再取回候選、整理證據並附來源回答](/upstream/resources/diagrams/rag-pipeline-overview.png)
...
都能做到後，前往 [Stage 7 — Agent Production Engineering：Harness、Loop 與 Graph](/zh-tw/stages/07-multi-agent-production/)。
