...
![Agent 系统选择图：先分辨由程序或 Agent 决定下一步，再看需要一个或多个 Agent，最后先选最简单能完成任务的形状](/upstream/resources/diagrams/agent-framework-choice-map.zh-Hans.png)
...
- Claude Code subagent 是 runtime 内置的另一条路：用配置文件隔离 context 与工具，不必自己写 Python orchestration。完整比较留在 [Stage 5.5](/stages/05-claude-code-ecosystem/#55--subagentsclaude-code-原生-multi-agent-机制-2025-新功能)。
...
都做到后，进入 [Stage 5 — Claude Code Ecosystem](/stages/05-claude-code-ecosystem/)。如果还分不清四格，回到上面的选择地图；不必重读 19 笔表格。
