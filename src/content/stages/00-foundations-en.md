...
description: "This stage checks four tools that you will use later. If you already know them, skip ahead. If not, no problem: do the small practice below once."
section: "Stage"
...
If all four are easy, go straight to [Stage 1 — LLM Basics](/en/stages/01-llm-basics/). If one is uncertain, do the main practice here. Open the extra sections only when you need them.
...
When all four are done, go to [Stage 1 — LLM Basics](/en/stages/01-llm-basics/). If you are stuck, open the section below that matches the problem. You do not need to read everything.
...
For other unfamiliar words, check the [glossary](/en/resources/glossary/). To see why an agent may appear in a terminal, a chat app, or a device, read the [Agent Landscape](/en/resources/agent-paradigms/). Neither is required before the main practice.
...
> ✅ **Done with Stage 0?** Continue to [**Stage 1 — LLM Basics**](/en/stages/01-llm-basics/) to make your first LLM API call and learn about token, context window, and cost estimation.
