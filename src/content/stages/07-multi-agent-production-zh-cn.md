---
title: "Stage 7 — Agent Production Engineering：Harness、Loop 与 Graph"
description: "这一关要做的是 Agent Production Engineering（Agent 上线工程）：先用 Eval 证明结果真的正确，再用 Observability 看见过程，接着加入人工批准、Checkpoint 和恢复，最后才部署。它不只要“偶尔成功”，还要能被检查、能安全停下，也能从正确位置继续。"
section: "Stage"
...
![Agent Eval 证据循环：定义 Case、Suite 和参考标准；用多次 Trial 与 Grader 比较 Baseline；迭代时使用 Development Cases，发布候选版本才打开 Frozen Holdout，并把去标识化失败案例加入下一版 Suite](/upstream/resources/diagrams/eval-evidence-loop.zh-Hans.png)

## 🚪 进入条件

你至少应该完成：

- [Stage 4](/stages/04-agent-frameworks/)：知道 Agent、Tool 和 Workflow 是什么。
- [Stage 5](/stages/05-claude-code-ecosystem/)：看过工具权限、Subagent 和开发流程。
- [Stage 6](/stages/06-memory-rag/)：知道 Context、RAG 和 Memory 不一样。
...
|---|---|---|---|---|---|
| 1 | 我有没有把话说清楚？ | **Prompt** | **Prompt Engineering** | [Stage 2](/stages/02-prompt-engineering/) | 每章的 Prompt 和 Eval |
| 2 | 我有没有把该看的资料放进来？ | **Context** | **Context Engineering** | [Stage 2](/stages/02-prompt-engineering/) 先分清 Prompt 与 Context | [Stage 6](/stages/06-memory-rag/) 的 RAG／Memory |
| 3 | 它能不能安全地使用工具、出错后停下？ | **Agent Harness** | **Harness Engineering** | [Stage 3](/stages/03-tool-use-and-hello-agent/) 的 runner／tool boundary | [Stage 5](/stages/05-claude-code-ecosystem/) 的实例与本章的 production checklist |
| 4 | 它怎么“做、看、再做”，又不会无限运行？ | **Agent Loop**；外层可重跑 Harness | **Loop Engineering**：长任务的目标、证据、调整与停止 | [Stage 3](/stages/03-tool-use-and-hello-agent/) | 本章的长任务 loop |
| 5 | 每一步、分支和返回路线能不能被看见和控制？ | **Workflow Graph** | **Production orchestration**；新兴文章也会写 Graph Engineering | [Stage 4](/stages/04-agent-frameworks/) | 本章的 production orchestration |
...
![一次 Agent run 和整个长任务：Harness 内含 Agent Loop；Workflow Graph 排整条路线，Loop Engineering 根据证据调整](/upstream/resources/diagrams/agent-engineering-control-questions.zh-Hans.png)
...
完成后，进入 [Stage 7.5 — 进阶 Agentic 概念地图](/stages/07-5-advanced-agentic-concepts/)，再到 [Stage 8 — Agent Interfaces](/stages/08-agent-interfaces/)。如果其中一项还说不清楚，回到对应练习，只改一件事再测试一次。
