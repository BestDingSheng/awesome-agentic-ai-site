---
title: "Stage 7 — Loop／Graph Engineering：多 Agent 与稳定运行"
description: "这一关要做的事很简单：让 Agent 不只“偶尔成功”，而是能被看见、被检查，出错时也能安全停下来。"
section: "Stage"
...
|---|---|---|---|---|
| 1 | 我有没有把话说清楚？ | **Prompt Engineering** | [Stage 2](/stages/02-prompt-engineering/) | 每章的 Prompt 和 Eval |
| 2 | 我有没有把该看的资料放进来？ | **Context Engineering** | [Stage 2](/stages/02-prompt-engineering/) 先分清 Prompt 与 Context | [Stage 6](/stages/06-memory-rag/) 的 RAG／Memory |
| 3 | 它能不能安全地使用工具、出错后停下？ | **Harness Engineering** | [Stage 3](/stages/03-tool-use-and-hello-agent/) 的 runner／tool boundary | [Stage 5](/stages/05-claude-code-ecosystem/) 的实例与本章的 production checklist |
| 4 | 它怎么“做、看结果、再做”，而且不会无限运行？ | **Loop Engineering** | [Stage 3](/stages/03-tool-use-and-hello-agent/) 的 Agent Loop | 本章的 bounded long-running loop |
| 5 | 每一步、分支和返回路线能不能被看见和控制？ | **Graph Engineering** | [Stage 4](/stages/04-agent-frameworks/) 的 Workflow Graph | 本章的 production orchestration |
