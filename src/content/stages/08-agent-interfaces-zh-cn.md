---
title: "Stage 8 — Agent 操作介面（Agent Interfaces）：Computer Use · Browser Use · Code Sandbox"
description: "👥 共享中心——与 Stage 5（Claude Code 生态系统）一样，本章是 Track A（CLI 高级用户）和 Track B（Agent 构建者）两条路径的共享中心。Stage 5 和 Stage 8 是本课程的两个核心枢纽。"
section: "Stage"
...
> [繁體中文](/zh-tw/stages/08-agent-interfaces/) | **简体中文** | [English](/en/stages/08-agent-interfaces/)
...
> 🔑 **关键词**：见本章内部解释 + [`resources/glossary.zh-Hans.md`](/resources/glossary/)
...
- 完成 [Stage 5](/stages/05-claude-code-ecosystem/)（了解 MCP / Skills / Plugins，日常使用 Claude Code）。
- 完成 [Stage 7](/stages/07-multi-agent-production/)（了解 harness engineering，知道 reward-hacking 警告的含义）。
...
**为什么真实能力 ≠ 数据**（呼应 [Stage 7 reward-hacking 警告](/stages/07-multi-agent-production/#-agent-benchmark-landscape怎么看不要只看排行榜---reward-hacking-警告)）：
...
- 之前都依赖 [Claude Code](/stages/05-claude-code-ecosystem/) / [Cursor](https://www.cursor.com) 的审批门来阻止——但生产级智能体**无人值守，必须使用沙箱**。
...
**为何选择 MCP 路线**：你已熟悉 Claude Code（[Stage 5](/stages/05-claude-code-ecosystem/)），新功能可通过 MCP 接入，无需更换工具。
...
→ **呼应 [Stage 7 reward-hacking 警告](/stages/07-multi-agent-production/#-agent-benchmark-landscape怎么看不要只看排行榜---reward-hacking-警告)**：课程始终强调“**不要盲目相信智能体**”的规范——Stage 7 讲评估规范，Stage 8 讲运行时规范。
...
如果都可以 → 你已完成课程主干。选择一个[特化分支](/#-学习地图两条学习路径)，或继续看下一节 下一个前沿。
...
1. **选择一个专业分支**（[面向研究人员](/branches/for-researcher/) / [面向开发者](/branches/for-developer/) / [面向教师](/branches/for-teacher/) / [面向知识工作者](/branches/for-knowledge-worker/) / [面向日常用户](/branches/for-everyday-users/)）
