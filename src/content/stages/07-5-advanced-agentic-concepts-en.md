---
title: "Stage 7.5 — Advanced Agentic Choices: Add Complexity Only When Evidence Calls for It"
description: "← Return to Stage 7 — Agent Production Engineering first if you cannot yet use Eval, Trace, Approval, and Recovery to show that your system is testable, observable, stoppable, and recoverable. Do not add the advanced cho"
section: "Stage"
...
← **Return to [Stage 7 — Agent Production Engineering](/en/stages/07-multi-agent-production/) first** if you cannot yet use Eval, Trace, Approval, and Recovery to show that your system is testable, observable, stoppable, and recoverable. Do not add the advanced choices in this chapter yet.
...
Finish [Stage 7](/en/stages/07-multi-agent-production/), and have a rerunnable set of Eval cases, stop conditions, and a single-Agent baseline. This stage does not require coding, buying API access, or revealing a model’s private Chain-of-Thought.
...
![Advanced agentic choices: from failure evidence to the smallest necessary approach](/upstream/resources/diagrams/advanced-agentic-decision-map.en.png)
...
![Model–Harness Fit: use the same Evals to keep, simplify, or remove a component](/upstream/resources/diagrams/model-harness-fit.en.png)
...
When all five are true, continue to [Stage 8 — Agent Interfaces](/en/stages/08-agent-interfaces/). If you do not yet have a baseline or failure evidence, return to [Stage 7](/en/stages/07-multi-agent-production/) and add them instead of adding another Agent.
