...
description: "This stage is not about memorizing every new term. Think of it as a map: start with the small area where your agent is stuck."
section: "Stage"
...
[Stage 7](/en/stages/07-multi-agent-production/) is the entry point. If you read selectively, first build an agent with a tool, an Eval, and a stopping condition.
...
- **Prerequisites:** Agent, Workflow, Handoff, Eval, Observability, and Guardrail. Review the [Stage 7 core terms](/en/stages/07-multi-agent-production/) if needed.
...
![12 advanced Agentic concepts grouped by problem](/upstream/resources/diagrams/concept-cluster.en.png)
...
![Start with the group that matches your problem](/upstream/resources/diagrams/reading-decision-tree.en.png)
...
|---|---|---|
| Repo state and diff | Know what changed and be able to recover. | [Stage 5](/en/stages/05-claude-code-ecosystem/) |
| Isolated execution environment | New code must run without contaminating the host. | [Stage 8](/en/stages/08-agent-interfaces/) |
| Long-task artifact hand-off | A new session must know where the previous one stopped. | [Stage 7](/en/stages/07-multi-agent-production/) |
...
![Model–Harness Fit: use the same Eval to keep, simplify, or remove](/upstream/resources/diagrams/model-harness-fit.en.png)
...
After all five, continue to [Stage 8 — Agent Interfaces](/en/stages/08-agent-interfaces/): learn how Agents safely operate browsers, computers, and code sandboxes.
