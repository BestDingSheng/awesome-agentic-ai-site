---
title: "Stage 3 — 工具使用與第一個 Agent Loop ⭐"
description: "這一關要做一件事：讓模型填一張「工具工作單」，再由你的程式檢查、執行並把結果送回去。這個來回就是你的第一個 Agent Loop。"
section: "Stage"
...
🌐 [English](/en/stages/03-tool-use-and-hello-agent/) | [简体中文](/stages/03-tool-use-and-hello-agent/) | **繁體中文**
...
你能執行一個 Python 檔、看懂 function 與 dict，並完成 [Stage 02](/zh-tw/stages/02-prompt-engineering/)，就可以開始。環境還沒好時，先回 [Stage 00](/zh-tw/stages/00-foundations/)。
...
![Tool Use 六步圖：模型提出 Tool Call，程式驗證並執行，再把 Tool Result 送回模型。](/upstream/resources/diagrams/tool-use-loop.png)
...
**先備知識**：能執行 Python、看懂 list／dict／function，並完成 [Stage 02](/zh-tw/stages/02-prompt-engineering/)。
...
| 名詞 | 白話用途 | 放在哪裡學 |
|---|---|---|
| **Chain-of-Thought（CoT）** | 早期 prompt 技巧常要求寫出中間推理。現在不把完整私人思維鏈當成通用輸出要求；需要檢查時，看最後答案與簡短、可驗證的理由 | [Stage 02](/zh-tw/stages/02-prompt-engineering/) |
| **ReAct** | 在迴圈中交替採取 action、讀 observation、再決定下一步 | 本章練習 3 |
| **Reflection** | 用一次回饋改下一次嘗試的廣義做法 | 本章下方路由 |
| **Reflexion／Self-Refine** | 有明確 Actor／Critic 或自我回饋流程的研究 pattern | 本章概念；持久記憶版到 [Stage 06](/zh-tw/stages/06-memory-rag/) |
| **Planning** | 先拆成多步，再依結果調整計畫 | [Stage 07.5](/zh-tw/stages/07-5-advanced-agentic-concepts/) |
...
如果你偏好用檔案實作，可直接開啟[練習 1 完整資料夾](/zh-tw)。
...
- [Path A README（Ollama）](/zh-tw)：執行 `python starter.py`。
...
真正的程式還要把 assistant 的 Tool Call 放回 history，並處理 refusal、max tokens、timeout、未知工具、JSON 解析與工具例外。完整雙路徑在 [`03-react-from-scratch`](/zh-tw)。
...
完整雙路徑在 [`04-multi-step-reasoning`](/zh-tw)。比較模型時，固定 prompt、tools、schema、`MAX_STEPS` 和測試題，至少重跑五次，再記成功率與失敗類型。
...
完整雙路徑在 [`05-error-handling`](/zh-tw)。Path A 的 API 費用是 `$0`；Path B 一輪錯誤復原先保留 `$0.10`。
...
完整雙路徑在 [`06-schema-design`](/zh-tw)，速查表在 [`resources/schema-design-cheatsheet.md`](/zh-tw/resources/schema-design-cheatsheet/)。
...
本章只理解 single-session loop。需要跨 session 保存失敗經驗時，進 [Stage 06 的 Reflection Memory](/zh-tw/stages/06-memory-rag/)；需要更完整的 planning、verification 與長時執行，進 [Stage 07.5](/zh-tw/stages/07-5-advanced-agentic-concepts/)。
...
都做到後，進入 [Stage 4 — Workflow Graph 與 Agent 框架](/zh-tw/stages/04-agent-frameworks/)。如果還說不出完整來回，先重跑練習 1；不需要把整章重新讀一遍。
