...
description: "This stage teaches only three things: say what you mean, give examples, and check answers."
section: "Stage"
...
- **Prerequisite**: Finish [Stage 1](/en/stages/01-llm-basics/) and be able to run a Python program.
...
> ▶️ Want to run it directly? See [`examples/stage-2/01-prompt-eval-loop/`](/en).
...
Labels help organize content, but they are not a complete security wall. Put formal prompt-injection protection in [Stage 8](/en/stages/08-agent-interfaces/).

### Optional 3: Strict JSON needed

Writing only “return JSON” does not guarantee valid JSON every time. The program must report a clear error when parsing fails. When you need a fixed schema, use Structured Outputs or a tool schema from [Stage 3](/en/stages/03-tool-use-and-hello-agent/).
...
| Context Engineering | Which data to put into the context window this time | [Stage 6](/en/stages/06-memory-rag/) |
| Harness Engineering | The loop, retry, sandbox, eval, and observability around the model | [Stage 7](/en/stages/07-multi-agent-production/) |
