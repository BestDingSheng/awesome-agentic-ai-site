---
title: "Stage 5 — Claude Code 生态系统 ⭐⭐"
description: "Claude Code 是一位会使用文件和终端的助手。本章教你如何为它设置规则、工具和安全边界，而不是让你一次装完所有东西。"
section: "Stage"
...
![Claude Code 扩展工具选择图](/upstream/resources/diagrams/claude-code-extension-map.zh-Hans.png)
...
> 想分清 OpenRouter、Pi、OpenCode 和 Ollama？OpenRouter 是 **Router**，Ollama 是 **本地运行时**，Claude Code、OpenCode 和 Pi 是 **编码代理／harness**。完整选择表见 [Track A1](/tracks/cli-a1-cli-intro/)。

## 🚪 进入条件与阅读路径

- **Track A（CLI 用户）**：完成 [A2](/tracks/cli-a2-cli-workflow/) 后读 5.1–5.4，掌握项目守则、Skill、MCP 和 Plugin，再前往 [A3](/tracks/cli-a3-cli-production/)。
- **Track B（代理开发者）**：完成 [Stage 3](/stages/03-tool-use-and-hello-agent/) 和 [Stage 4](/stages/04-agent-frameworks/) 后，再读 5.5–5.8。
...
![Claude Code 5.1–5.7 关系图：CLAUDE.md 与 Skill 提供 context，Agent loop 通过 MCP 使用外部工具，Hook 按事件检查，Subagent 与 Worktree 分别隔离 context 和文件，Plugin 只负责打包](/upstream/resources/diagrams/claude-code-system-flow.zh-Hans.png)
...
15 个进阶可复制 recipe：[`resources/subagent-cookbook.zh-Hans.md`](/resources/subagent-cookbook/)。组合与排错：[`resources/subagent-advanced.zh-Hans.md`](/resources/subagent-advanced/)。
...
把答案画成六格箭头图，再用 100–150 字比较 [Stage 3 的最小 ReAct loop](/stages/03-tool-use-and-hello-agent/) 增加了哪些控制边界。
...
如果可以，按你的路线前进：**Track A** 前往 [A3 — 安全的团队流程](/tracks/cli-a3-cli-production/)；**Track B** 前往 [Stage 6 — Memory & RAG](/stages/06-memory-rag/)。如果还不行，回到“一张表先选对组件”，只重做你分不清的那一行。
