...
description: "| Layer | 是什么 | Claude 的版本 | 谁管 | 学在 | |---|---|---|---|---| | L7 Interface | 用户和 agent 交谈入口 | claude-code CLI / Desktop | Harness Engineering | Stage 5.1 | | L6 Workflow | 固定可复用流程模板 | Skills（SKILL.md）+ Slash commands + "
section: "Stage"
...
> 🚪 **进入条件与两条轨**（本 stage 跟 [Stage 8 — Agent Interfaces](/stages/08-agent-interfaces/) 是 curriculum 的两个共用 hub）：
...
> - **Track B（Agent Builder）**：建议先完成 [Stage 3](/stages/03-tool-use-and-hello-agent/)（tool use）+ [Stage 4](/stages/04-agent-frameworks/)（agent frameworks）再进，把整个 stage 当“**Claude Code 内部怎么运作**”深读，从 5.1 完整走到 5.7。

> 📋 **本章组成**：7 个核心子章（5.1 基础 / 5.2 MCP / 5.3 Skills / 5.4 Plugins / 5.5 Subagents / 5.6 Dynamic Workflows / 5.7 Claude Code Source 解剖）+ 5.8 SDK（选修、包成产品或服务才需要）。每个子章都是“学习目标 → 必修阅读 → 动手练习 → 精选 Projects”，章末有自我检查。整个 stage 围绕 4 个关键词：**MCP / Skills / Plugins / Marketplace**——不熟先翻 [`resources/glossary.zh-Hans.md` 5](/resources/glossary/#5-claude-code-生态)。
...
Stage 5 接下来专讲 Claude Code。想看完整 5 种 agent paradigm 对照 → [`resources/agent-paradigms.zh-Hans.md`](/resources/agent-paradigms/)（Claude Code 是 Type 1 IDE-coupled + Type 2 Terminal pair-programmer）。
...
> 看 [`resources/mcp-skills-catalog.zh-Hans.md`](/resources/mcp-skills-catalog/)——按 16 个分类整理 81+ 个常用 MCP server / Skill，每个都附 stars / license / 适合谁。下表保留的是“**写自己 MCP server 时的 reference**”性质的官方 server / SDK。
...
> 📋 **准备自己写 subagent / 组合多个 / debug 跑坏的？** → [`resources/subagent-advanced.zh-Hans.md`](/resources/subagent-advanced/)（description 写法 4 个 bug 对照、composition 3 种 pattern、debug 5 切点）
...
> **discipline 级概念在哪**：harness engineering 是什么 / framework vs harness 差别 / prompt→context→harness 五层工程分工 → 全部见 **[Stage 7 Harness Engineering](/stages/07-multi-agent-production/#-harness-engineering--production-agent-runtime-的工程设计--本-stage-核心概念)**。本节只负责 Claude Code source 的 case study。
