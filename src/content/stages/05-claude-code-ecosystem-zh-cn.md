...
> 看 [`resources/mcp-skills-catalog.zh-Hans.md`](/resources/mcp-skills-catalog/)——按 14 个分类整理 62 个常用 MCP server / Skill，每个都附 stars / license / 适合谁。下表保留的是“**写自己 MCP server 时的 reference**”性质的官方 server / SDK。
...
| Project | ⭐ | 适合谁 | 为什么推荐 / 备注 |
|---|---|---|---|
| [anthropics/skills](https://github.com/anthropics/skills) ⭐ 官方 spec | ⭐⭐⭐⭐⭐ | 写自己 SKILL.md 前先读 | Anthropic 官方 Skills repo：`spec/`（frontmatter 标准）+ `template/` 起手范本 + `skills/` 含 pdf / docx / xlsx / pptx / skill-creator / skill-vetter 等 reference 实现。★ 128k+。**SKILL.md 结构范本参考**。Agent Skills 更广义标准另见 [agentskills.io](https://agentskills.io) |
...
![Subagent 的 4 个生命周期：从 .md 文件到执行结果](/upstream/resources/diagrams/subagent-4-stage-flow.png)
...
![Subagent vs Skill — 5 个关键差别](/upstream/resources/diagrams/subagent-vs-skill.png)
...
- [ ] **读过 `claude-agent-sdk-python` 的 main loop、能在 source 里标出 [Stage 7 列的 8 个 harness 元件](/stages/07-multi-agent-production/#-harness-engineering--production-agent-runtime-的工程设计--本-stage-核心概念) 的前 6 个 runtime-internal 元件**位置（5.6 练习）
