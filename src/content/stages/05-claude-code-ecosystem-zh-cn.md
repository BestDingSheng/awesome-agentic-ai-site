...
> 💡 整个 stage 围绕 4 个关键词（**MCP / Skills / Plugins / Marketplace**）展开 → 不熟先翻 [`resources/glossary.zh-CN.md` §5](/resources/glossary/#5-claude-code-生态)。
...
- **练习：MCP server** — 写一个 Python MCP server，提供一个 tool（例如「换算温度」）。从 Claude Code 连过去。**step-by-step 怎么做** → [`resources/cookbook.zh-CN.md` §2](/resources/cookbook/#2-写你的第一个-mcp-server)
...
> 看 [`resources/mcp-skills-catalog.zh-CN.md`](/resources/mcp-skills-catalog/)——按 14 个分类整理 62 个常用 MCP server / Skill，每个都附 stars / license / 适合谁。下面这节保留的是「**写自己 MCP server 时的 reference**」性质的官方 server / SDK。
...
### 动手练习
- **练习：SKILL.md** — 写一份 200 字的 skill，解决你日常工作中的某一件事。**step-by-step 怎么做** → [`resources/cookbook.zh-CN.md` §1](/resources/cookbook/#1-写你的第一个-skill)
...
> 看 [`resources/mcp-skills-catalog.zh-CN.md`](/resources/mcp-skills-catalog/)——按使用场景分类，含 Anthropic 官方 + 社区 Skill。下面这节保留的是「**写自己 Skill 时的 reference**」性质的 spec / showcase。
...
> 💡 **想看别人的 plugin 怎么包**：[`resources/mcp-skills-catalog.zh-CN.md`](/resources/mcp-skills-catalog/) 的开发协作 / 设计 / 监控分类底下不少都附 plugin 包装（例如 `timescale/pg-aiguide` 同时是 MCP 跟 plugin）。下面这节保留的是「**marketplace 结构模板**」性质的 reference。
