---
title: "Stage 5 — Claude Code 生态系（Claude Code Ecosystem）⭐⭐"
description: "👥 共用 hub：本 stage 是 Track A（CLI Power User）+ Track B（Agent Builder）两条路径的共用中心。Stage 5 跟 Stage 8 — Agent Interfaces 是 curriculum 的两个 hub。"
section: "Stage"
...
> [繁體中文](/zh-tw/stages/05-claude-code-ecosystem/) | **简体中文** | [English](/en/stages/05-claude-code-ecosystem/)
...
![Claude Code Ecosystem Stack](/upstream/resources/diagrams/stage5-stack.zh-Hans.png)
...
![Claude Code 7-Layer Architecture Map](/upstream/resources/diagrams/claude-architecture-map.zh-Hans.png)
...
| **L4 Memory / Context** | 跨对话 / 跨 session 记事情 | History / `/compact` / Memory hooks | Context Engineering | [Stage 6](/stages/06-memory-rag/) |
...
| **L2 Tool Use** | LLM 调用外部 function 的 protocol | Anthropic Tool Use（`input_schema`）| Tool design | [Stage 3](/stages/03-tool-use-and-hello-agent/) |
...
| **L1 Foundation** | LLM 本体（system prompt 直接送达这一层）| Anthropic API | Prompt Engineering | [Stage 1](/stages/01-llm-basics/) + [Stage 2](/stages/02-prompt-engineering/) |
...
|---|---|---|---|
| **Prompt Engineering** | L1 + L6 | "送进 LLM 的字符串怎么设计" | [Stage 2](/stages/02-prompt-engineering/) |
| **Context Engineering** | L4 + L2.5 | "context window 装什么信息" | [Stage 6](/stages/06-memory-rag/) |
| **Harness Engineering** | L3 + L5 + L7 | "LLM 外面的'运行外壳'——给它工具、记忆、控制流程的那层设置" | [Stage 7 §Harness Engineering](/stages/07-multi-agent-production/#-harness-engineering--production-agent-runtime-的工程设计--本-stage-核心概念) |
...
→ 细看 [`resources/cli-agents-guide.zh-Hans.md`](/resources/cli-agents-guide/)
...
> 🛠️ **要写好 CLAUDE.md？** 先看 [Stage 7.5 核心 Harness Engineering 原则（多 source 整理）](/stages/07-5-advanced-agentic-concepts/#-跨概念-harness-engineering-原则多-source-整理) 建概念、再用下面 2 个 prompt 动手。
...
- **练习：MCP server** — 写一个 Python MCP server，提供一个 tool（例如“换算温度”）。从 Claude Code 连过去。**step-by-step 怎么做** → [`resources/cookbook.zh-Hans.md` 2](/resources/cookbook/#2-写你的第一个-mcp-server)
...
> 看 [`resources/mcp-skills-catalog.zh-Hans.md`](/resources/mcp-skills-catalog/)——按 16 个分类整理 65+ 个常用 MCP server / Skill，每个都附 stars / license / 适合谁。下表保留的是“**写自己 MCP server 时的 reference**”性质的官方 server / SDK。
...
> - **路 B：用下面 SKILL.md 设计 prompts 自己写**——先看 [Stage 7.5 核心 Harness Engineering 原则](/stages/07-5-advanced-agentic-concepts/#-跨概念-harness-engineering-原则多-source-整理) 建概念、再用 prompt 动手。
>
...
### 动手练习
- **练习：SKILL.md** — 写一份 200 字的 skill，解决你日常工作中的某一件事。**step-by-step 怎么做** → [`resources/cookbook.zh-Hans.md` 1](/resources/cookbook/#1-写你的第一个-skill)
...
> 看 [`resources/mcp-skills-catalog.zh-Hans.md`](/resources/mcp-skills-catalog/)——按使用情境分类，含 Anthropic 官方 + 社区 Skill。下表保留的是“**写自己 Skill 时的 spec / showcase reference**”性质。

| Project | ⭐ | 适合谁 | 为什么推荐 / 备注 |
|---|---|---|---|
| [anthropics/skills](https://github.com/anthropics/skills) ⭐ 官方 spec | ⭐⭐⭐⭐⭐ | 写自己 SKILL.md 前先读 | Anthropic 官方 Skills repo：`spec/`（frontmatter 标准）+ `template/` 起手范本 + `skills/` 含 pdf / docx / xlsx / pptx / skill-creator / skill-vetter 等 reference 实现。★ 158k+。**SKILL.md 结构范本参考**。Agent Skills 更广义标准另见 [agentskills.io](https://agentskills.io) |
...
| [mattpocock/skills](https://github.com/mattpocock/skills) | ⭐⭐⭐⭐ | 想看“真实工程师日常 SKILL.md” | Matt Pocock（TypeScript 社区知名教学者）公开自己工作真正在用的 `.claude/` 目录。每个 SKILL.md **10-50 行极短**、不过度工程化。**对照 over-engineered 200 行 skill 特别有参考价值**（★ 178k+、MIT）|
...
4. 想看更多 → 逛 `awesome-claude-code-toolkit` 或 [`resources/mcp-skills-catalog.zh-Hans.md`](/resources/mcp-skills-catalog/)
...
| [obra/superpowers-marketplace](https://github.com/obra/superpowers-marketplace) | ⭐⭐⭐⭐ | 想做“我策展、别人写”型 marketplace | **最简 marketplace template**——repo 只有 `marketplace.json` + README、plugin 本体放外部 repo。curator-only pattern 最小范本（★ 1.1k+、MIT）|
...
| [anthropics/life-sciences](https://github.com/anthropics/life-sciences) | ⭐⭐⭐ | 要做特定领域 marketplace（医疗、金融、法律、教育等） | Anthropic 自家**领域特化 marketplace** 范例（生物 / 健康科学）、展示 `marketplace.json` 为单一 vertical 量身设计。**payload 偏生科 MCP server、marketplace.json 结构才是学习重点**（★ 523）|
...
![Subagent 的 4 个生命周期：从 .md 文件到执行结果](/upstream/resources/diagrams/subagent-4-stage-flow.zh-Hans.png)
...
| **Framework path**<br>（Stage 4） | LangGraph / CrewAI / AutoGen | ✅ 自己 wire | 部分 | 跨 LLM provider、Python orchestration、见 [Stage 4](/stages/04-agent-frameworks/) |
...
> 📋 **完整 15 个 recipe**（每个含**场景 + subagent + 直接复制粘贴的 prompt 模板 + 何时不用**）→ [`resources/subagent-cookbook.zh-Hans.md`](/resources/subagent-cookbook/)
...
![Subagent vs Skill — 5 个关键差别](/upstream/resources/diagrams/subagent-vs-skill.zh-Hans.png)
...
| **Debug 多一层** | 出错不知道该怪主 session description / subagent system prompt / 还是 prompt 本身——见 [advanced §3 debug 5 切点](/resources/subagent-advanced/#3-自制-subagent-的-debug-工具)|
...
> **本节定位**：5.5 教你**手动**派 subagent；本节更上一层——**让 Claude 自己生成一份 workflow 脚本、再自己执行**。这是 Opus 4.8 起的新机制（research preview 出身），新版 Claude Code 内置。本节只把它放进生态地图、讲清楚跟 5.5 的分工；**机制 / 实例 / quality pattern 的完整版在 [Stage 7.5 — Dynamic Workflows 深入](/stages/07-5-advanced-agentic-concepts/#-dynamic-workflowsopus-48-当-agent-自己写出-workflow)**。
...
2. **[Stage 7.5 — Dynamic Workflows 深入](/stages/07-5-advanced-agentic-concepts/#-dynamic-workflowsopus-48-当-agent-自己写出-workflow)** ⭐ — 机制、quality pattern（adversarial verify / loop-until-dry / judge panel）、何时用的完整版
...
3. **标出前 6 个 runtime-internal harness 元件**在 source 里的位置（文件名 + 行号）——对应 [Stage 7 列的 8 元件](/stages/07-multi-agent-production/#-harness-engineering--production-agent-runtime-的工程设计--本-stage-核心概念)的前 6 个（第 7 个 Eval 外挂 / 第 8 个 Cost-Latency cross-cutting 不在 source 主 loop）：
...
| [ai-boost/awesome-harness-engineering](https://github.com/ai-boost/awesome-harness-engineering) | ⭐⭐⭐⭐ | 5.7 读完想扩大视野 | community curation：30+ harness / eval / memory / observability / MCP project（★ 3.2k+）。**广度资源库、非教程**——挑感兴趣的 sub-topic 钻进去 |
...
- [ ] **读过 `claude-agent-sdk-python` 的 main loop、能在 source 里标出 [Stage 7 列的 8 个 harness 元件](/stages/07-multi-agent-production/#-harness-engineering--production-agent-runtime-的工程设计--本-stage-核心概念) 的前 6 个 runtime-internal 元件**位置（5.7 练习）
- [ ] 从角色分工说出 MCP / Skills / Plugins / Subagents / SDK 各自的位置

如果都可以 → 前往 [Stage 6 — Memory & RAG](/stages/06-memory-rag/)。

> 💡 **Stage 5 是两 track 第一个 hub**——Track A 跟 Track B 都会用到。第二个 hub 是 [**Stage 8 — Agent Interfaces**](/stages/08-agent-interfaces/)（Computer Use / Browser Use / Sandbox），可以走完主干后再进、或对 Computer Use / Browser MCP 有兴趣可以提前 preview。
