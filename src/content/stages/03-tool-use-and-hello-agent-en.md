...
If you can run a Python file, understand functions and dicts, and have completed [Stage 02](/en/stages/02-prompt-engineering/), you are ready. If your environment is not ready, go back to [Stage 00](/en/stages/00-foundations/) first.
...
![Tool Use loop: the model proposes a Tool Call; the app validates and runs the tool; the Tool Result returns to the model.](/upstream/resources/diagrams/tool-use-loop.en.png)
...
|---|---|---|
| **Chain-of-Thought (CoT)** | Early prompt techniques often asked for intermediate reasoning. Do not treat a full private chain of thought as a general output requirement; when checking, look at the final answer and a short, verifiable reason | [Stage 02](/en/stages/02-prompt-engineering/) |
...
| **Reflexion / Self-Refine** | Research patterns with an explicit Actor/Critic or self-feedback process | This chapter’s concepts; persistent-memory version in [Stage 06](/en/stages/06-memory-rag/) |
| **Planning** | Break the task into steps, then adjust the plan from results | [Stage 07.5](/en/stages/07-5-advanced-agentic-concepts/) |
...
If you prefer working from files, open the [complete Exercise 1 folder](/en).
...
- [Path A README (Ollama)](/en): run `python starter.py`.
...
The real program must also put the assistant’s Tool Call back into history and handle refusal, max tokens, timeout, unknown tools, JSON parsing, and tool exceptions. The complete two paths are in [`03-react-from-scratch`](/en).
...
The complete two paths are in [`04-multi-step-reasoning`](/en). When comparing models, keep the prompt, tools, schema, `MAX_STEPS`, and test cases fixed; rerun at least five times and record success rates and failure types.
...
The complete two paths are in [`05-error-handling`](/en). Path A has `$0` in API cost; reserve `$0.10` for one Path B error-recovery round.
...
The complete two paths are in [`06-schema-design`](/en), with a quick reference in [`resources/schema-design-cheatsheet.en.md`](/en/resources/schema-design-cheatsheet/).
...
This chapter covers a single-session loop only. For carrying failed experiences across sessions, go to [Stage 06 Reflection Memory](/en/stages/06-memory-rag/); for fuller planning, verification, and long-running execution, go to [Stage 07.5](/en/stages/07-5-advanced-agentic-concepts/).
