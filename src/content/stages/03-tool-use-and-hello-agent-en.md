...
> 💡 Terminology-heavy (agent / tool use / function calling / ReAct / structured output) → See [`resources/glossary.md` 2](/zh-tw/resources/glossary/#2-agent--tool-use).
...
> 🔑 **Key Terms**: See [`resources/glossary.md` 2](/zh-tw/resources/glossary/#2-agent--tool-use)
...
>
> 💰 **Stage 3 Budget Estimate** (for all 6 exercises, with heavy tool use): **All local = $0**; **all haiku ≈ $0.50**; **all sonnet ≈ $1.50**. The ReAct loop exercise is about 4-6 tool calls × 5 exercises × 5 reps ≈ $0.80 on haiku. For the full budget, see [`examples/README.md#recommended-llms`](/en#recommended-llms).
>
> For the full 3-way trade-off, see [`examples/README.md`](/en#three-paths--defaulting-to-ollama-for-cost-reasons).
>
...
>
> 🪜 **This stage is the starting point for single-agent**: one LLM + ReAct loop. For **multi-agent concepts** (multiple agents collaborating), see [Stage 4 What is a multi-agent framework](/zh-tw/stages/04-agent-frameworks/#-what-is-a-multi-agent-framework); for **Claude's native subagent mechanism** (`.claude/agents/` + Task tool, no framework needed), see [Stage 5.5](/zh-tw/stages/05-claude-code-ecosystem/#55--subagents--claude-codes-native-multi-agent-mechanism-new-in-2025).
...
> 💡 **Want to see how reflection looks in a production agent?**: [Stage 5 5.6 Harness Internals](/zh-tw/stages/05-claude-code-ecosystem/#56--claude-code-source-dissection--reference-harness-implementation-track-b-must-read) dissects the Claude Code source where you can see it—the agent self-evaluates the patch after a tool call, goes back to fix problems, and commits after correction. **This is one of the core building blocks of modern production agents**.
