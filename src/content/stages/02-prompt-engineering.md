...
description: "Prompt（提示）不是只有一句問題。它是你交給模型的一整份任務包，可以放進指令、要處理的資料、範例，以及輸出規則。"
section: "Stage"
...
![Prompt Engineering 一張圖看懂：Prompt 四格、範例數量、檢查迴圈，以及不要求完整內部想法的 CoT 可檢查步驟](/upstream/resources/diagrams/prompt-engineering-map.png)
...
- **先備**：完成 [Stage 1](/zh-tw/stages/01-llm-basics/)，並能執行一段 Python。
...
> ▶️ 想直接跑一遍？看 [`examples/stage-2/01-prompt-eval-loop/`](/zh-tw)。
...
標籤能幫忙整理內容，但不是完整的安全牆。正式的 prompt injection 防護放在 [Stage 8](/zh-tw/stages/08-agent-interfaces/)。

### 選修 3：需要嚴格 JSON

只寫「請回 JSON」不能保證每次都合法。程式必須在解析失敗時明確報錯。需要固定 schema 時，改用 [Stage 3](/zh-tw/stages/03-tool-use-and-hello-agent/) 的 Structured Outputs 或 tool schema。
...
| Context Engineering | 這一次要把哪些資料放進 context window | [Stage 6](/zh-tw/stages/06-memory-rag/) |
| Harness Engineering | 模型外面的 loop、retry、sandbox、eval 與觀測 | [Stage 7](/zh-tw/stages/07-multi-agent-production/) |
...
都做到後，進入 [Stage 3 — 工具使用與第一個 Agent Loop](/zh-tw/stages/03-tool-use-and-hello-agent/)。
