...
description: "兩 track 共用 hub——跟 Stage 5（Claude Code 生態）一樣、Track A（CLI Power User）+ Track B（Agent Builder）兩條路徑都會用到。Stage 5 + Stage 8 是 curriculum 兩個 hub。"
section: "Stage"
...
> 🔑 **關鍵名詞**：見本章內部解釋 + [`resources/glossary.md`](/zh-tw/resources/glossary/)
...
- 完成 [Stage 5](/zh-tw/stages/05-claude-code-ecosystem/)（懂 MCP / Skills / Plugins、Claude Code 用過 daily）
- 完成 [Stage 7](/zh-tw/stages/07-multi-agent-production/)（懂 harness engineering、knows what reward-hacking warning is about）
...
**Why 真實能力 ≠ 數字**（呼應 [Stage 7 reward-hacking 警告](/zh-tw/stages/07-multi-agent-production/#-agent-benchmark-landscape怎麼看不要只看排行榜---reward-hacking-警告)）：
...
- 之前都靠 [Claude Code](/zh-tw/stages/05-claude-code-ecosystem/) / [Cursor](https://www.cursor.com) 的 approval gate 擋——但 production agent **無人盯、必須 sandbox**
...
→ **呼應 [Stage 7 reward-hacking 警告](/zh-tw/stages/07-multi-agent-production/#-agent-benchmark-landscape怎麼看不要只看排行榜---reward-hacking-警告)**：curriculum 一致教「**別 blindly 信 agent**」紀律——Stage 7 講 eval 紀律、Stage 8 講 runtime 紀律。
...
1. **挑一個 specialist branch**（[for-researcher](/zh-tw/branches/for-researcher/) / [for-developer](/zh-tw/branches/for-developer/) / [for-teacher](/zh-tw/branches/for-teacher/) / [for-knowledge-workers](/zh-tw/branches/for-knowledge-worker/) / [for-everyday-users](/zh-tw/branches/for-everyday-users/)）
