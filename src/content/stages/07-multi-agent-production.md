...
**4 個明確訊號**才上 multi-agent（詳見 [Stage 4 什麼時候真的需要 multi-agent](/zh-tw/stages/04-agent-frameworks/#什麼時候真的需要-multi-agent不要硬上)）：
...
如果都可以 → 進 [**Stage 8 — Agent Interfaces**](/zh-tw/stages/08-agent-interfaces/)（**兩 track 共用 hub**）學 agent 怎麼跟非 API 世界互動（Computer Use / Browser Use / Sandbox）。或挑一個[特化分支](/zh-tw#️-學習地圖兩條學習路徑)、或回過頭來貢獻這份 repo。
