---
title: "Stage 7 — Agent Production Engineering：Harness、Loop 與 Graph"
description: "這一關要做的是 Agent Production Engineering（Agent 上線工程）：先用 Eval 證明結果真的對，再用 Observability 看見過程，接著放入人工核准、Checkpoint 與復原，最後才部署。它不只要「偶爾成功」，還要能被檢查、能安全停下，也能從正確位置繼續。"
section: "Stage"
...
![Agent Eval 證據迴圈：先建立 Case、Suite 與參考標準，用多次 Trial 和 Grader 比較 Baseline；開發時只用 Development Cases，Release Candidate 才打開 Frozen Holdout，失敗案例去識別化後回到下一版 Suite](/upstream/resources/diagrams/eval-evidence-loop.png)

## 🚪 進入條件

你至少應該完成：

- [Stage 4](/zh-tw/stages/04-agent-frameworks/)：知道 Agent、Tool 與 Workflow 是什麼。
- [Stage 5](/zh-tw/stages/05-claude-code-ecosystem/)：看過工具權限、Subagent 與開發流程。
- [Stage 6](/zh-tw/stages/06-memory-rag/)：知道 Context、RAG 與 Memory 不一樣。
...
|---|---|---|---|---|---|
| 1 | 我有沒有把話說清楚？ | **Prompt** | **Prompt Engineering** | [Stage 2](/zh-tw/stages/02-prompt-engineering/) | 每章的 Prompt 與 Eval |
| 2 | 我有沒有把該看的資料放進來？ | **Context** | **Context Engineering** | [Stage 2](/zh-tw/stages/02-prompt-engineering/) 先分清 Prompt 與 Context | [Stage 6](/zh-tw/stages/06-memory-rag/) 的 RAG／Memory |
| 3 | 它能不能安全地用工具、出錯後停下？ | **Agent Harness** | **Harness Engineering** | [Stage 3](/zh-tw/stages/03-tool-use-and-hello-agent/) 的 runner／tool boundary | [Stage 5](/zh-tw/stages/05-claude-code-ecosystem/) 的實例與本章的 production checklist |
| 4 | 它怎麼「做、看、再做」，又不會無限跑？ | **Agent Loop**；外層可重跑 Harness | **Loop Engineering**：長任務的目標、證據、調整與停止 | [Stage 3](/zh-tw/stages/03-tool-use-and-hello-agent/) | 本章的長任務 loop |
| 5 | 每一步、分支與返回路線能不能被看見和控制？ | **Workflow Graph** | **Production orchestration**；新興文章也會寫 Graph Engineering | [Stage 4](/zh-tw/stages/04-agent-frameworks/) | 本章的 production orchestration |
...
![一次 Agent run 與整個長任務：Harness 內含 Agent Loop；Workflow Graph 排整條路，Loop Engineering 依證據調整](/upstream/resources/diagrams/agent-engineering-control-questions.png)
...
完成後，進入 [Stage 7.5 — 進階 Agentic 概念地圖](/zh-tw/stages/07-5-advanced-agentic-concepts/)，再到 [Stage 8 — Agent Interfaces](/zh-tw/stages/08-agent-interfaces/)。如果其中一項還說不清楚，回到對應練習，只改一件事再測一次。
