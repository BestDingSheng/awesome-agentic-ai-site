---
title: "Stage 7 — Loop／Graph Engineering：多 Agent 與穩定運作"
description: "這一關要做的事很簡單：讓 Agent 不只「偶爾成功」，而是能被看見、被檢查，出錯時也能安全停下來。"
section: "Stage"
...
|---|---|---|---|---|
| 1 | 我有沒有把話說清楚？ | **Prompt Engineering** | [Stage 2](/zh-tw/stages/02-prompt-engineering/) | 每章的 Prompt 與 Eval |
| 2 | 我有沒有把該看的資料放進來？ | **Context Engineering** | [Stage 2](/zh-tw/stages/02-prompt-engineering/) 先分清 Prompt 與 Context | [Stage 6](/zh-tw/stages/06-memory-rag/) 的 RAG／Memory |
| 3 | 它能不能安全地用工具、出錯後停下？ | **Harness Engineering** | [Stage 3](/zh-tw/stages/03-tool-use-and-hello-agent/) 的 runner／tool boundary | [Stage 5](/zh-tw/stages/05-claude-code-ecosystem/) 的實例與本章的 production checklist |
| 4 | 它怎麼「做、看結果、再做」，而且不會無限跑？ | **Loop Engineering** | [Stage 3](/zh-tw/stages/03-tool-use-and-hello-agent/) 的 Agent Loop | 本章的 bounded long-running loop |
| 5 | 每一步、分支與返回路線能不能被看見和控制？ | **Graph Engineering** | [Stage 4](/zh-tw/stages/04-agent-frameworks/) 的 Workflow Graph | 本章的 production orchestration |
