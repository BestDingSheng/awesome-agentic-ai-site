---
title: "A3 — Connect a CLI agent to a safe team workflow"
description: "This stop has one goal: have a CLI agent perform a read-only check on a test PR. It may give feedback, but it must not merge, deploy, or obtain extra permissions by itself."
section: "Track"
...
Choose Anthropic’s [`claude-code-action`](https://github.com/anthropics/claude-code-action) or OpenAI’s [`codex-action`](https://github.com/openai/codex-action). For the first round, run it only in a demo repo and branch you control, reusing A2’s [`review-changes` Skill](/en/tracks/cli-a2-cli-workflow/#cli-6).
...
For other situations, remember one action and keep the theory in [Stage 7.5](/en/stages/07-5-advanced-agentic-concepts/):
