---
title: "A3 — 把 CLI Agent 接進真實工作流程（Integration & Production）"
description: "CLI 跑得順了之後、下一步：把 CLI 接到你的真實團隊流程。這節達成 3 件事："
section: "Track"
...
> [← A2 — CLI Workflow Patterns](/zh-tw/tracks/cli-a2-cli-workflow/) · **Track A: CLI Power User** 第 3 站（最後）
...
>
> 完整定義見 [`resources/glossary.md` 5 + 6](/zh-tw/resources/glossary/#5-claude-code-生態)。
...
- 完成 [A1](/zh-tw/tracks/cli-a1-cli-intro/)：CLI 已選好、裝好、認證好
- 完成 [A2](/zh-tw/tracks/cli-a2-cli-workflow/)：寫過 production CLAUDE.md、會寫 slash command、跑過多步驟拆解
- 對 GitHub Actions / CI 基礎熟悉（會看 `.yml` workflow）
- 對 MCP 概念有印象（沒有的話先翻 [Stage 5.2](/zh-tw/stages/05-claude-code-ecosystem/#52--mcpmodel-context-protocol-基礎)）

沒到的話 → 補完 [A1](/zh-tw/tracks/cli-a1-cli-intro/) + [A2](/zh-tw/tracks/cli-a2-cli-workflow/)。A3 是「組合所有前面學的 → 接到 production」、跳級會看不懂。

## 📚 必修閱讀

1. [**Stage 5.2 — MCP（Model Context Protocol）**](/zh-tw/stages/05-claude-code-ecosystem/#52--mcpmodel-context-protocol-基礎) — MCP 概念跟基礎
...
3. [**Stage 7 — 常用 Multi-Agent / Production 工具推薦**](/zh-tw/stages/07-multi-agent-production/#-常用-multi-agent--production-工具推薦按用途分類) — langfuse / Helicone / weave 等 observability 工具表
4. [**`resources/cli-agents-guide.md`** 「常見坑」](/zh-tw/resources/cli-agents-guide/) — production 用 CLI 最常踩的問題

## 🛠 動手練習

### 動手練習 CLI-9：MCP server 接 CLI
照 [Stage 5.2 練習：MCP client](/zh-tw/stages/05-claude-code-ecosystem/#52--mcpmodel-context-protocol-基礎) 的步驟，把至少一個有用的 MCP server 接到你的 CLI：
...
- 接 langfuse 或 Helicone（[Stage 7 常用工具推薦](/zh-tw/stages/07-multi-agent-production/#-常用-multi-agent--production-工具推薦按用途分類) 表內 Observability 行）做 trace
...
- Skill / plugin 細節見 [Stage 5.3 + 5.4](/zh-tw/stages/05-claude-code-ecosystem/)
...
Track A 的人**已經在用** [Stage 7.5 的進階概念](/zh-tw/stages/07-5-advanced-agentic-concepts/)、只是沒命名它。下面挑 **最常用 2-3 個 playbook** 細看、其餘折疊為延伸閱讀——每個 ≤ 6 行。**想深挖原理 → 進 Stage 7.5。**
...
- **Read more**：
  ...
  | 內部 | [Stage 7.5 🧭 work boundary stack](/zh-tw/stages/07-5-advanced-agentic-concepts/#-概念地圖主軸四層工作邊界work-boundary) |
...
> 💡 **第一次聽到 subagent？** 一句話：**subagent = 主 Claude session spawn 出來的「子 Claude」**、有自己獨立的 context、跑完回報結果。**派遣（dispatch）**就是叫 subagent 去做事——像派任務給同事。完整概念 → [Stage 5.5](/zh-tw/stages/05-claude-code-ecosystem/#55--subagentsclaude-code-原生-multi-agent-機制-2025-新功能)。
...
- **Read more**：
  - [Stage 5.5 Subagents](/zh-tw/stages/05-claude-code-ecosystem/#55--subagentsclaude-code-原生-multi-agent-機制-2025-新功能)（完整理論 + decision table）
  - [`resources/subagent-cookbook.md`](/zh-tw/resources/subagent-cookbook/)（**15 個 recipe**、複製貼上即可用的 prompt 範本）
...
- **Read more**：
  ...
  | 內部 | [Stage 5.5 Subagents](/zh-tw/stages/05-claude-code-ecosystem/#55--subagentsclaude-code-原生-multi-agent-機制-2025-新功能) + 動手練習 CLI-10 |
...
- **Read more**：
  ...
  | 內部 | [Stage 7.5 🔁 failure-mode lifecycle](/zh-tw/stages/07-5-advanced-agentic-concepts/#-failure-mode-lifecycle產業級-agent-失敗模式怎麼演化成最佳實踐) |

---

→ **7 個 playbook = 7 個 trigger × 12 個 concept ×「對應 reading source」的橋樑**。深挖原理 / 看完整 12 個 concept 跟 8 個 cross-vendor 原則 → [Stage 7.5](/zh-tw/stages/07-5-advanced-agentic-concepts/)。

## 🎯 精選 Projects

按用途分 4 類、9 個項目一張表搞定。**挑入口看「適合誰」、想深入細節點連結看 repo**。

> 💡 **要找接日常工具的 MCP**（Notion / Obsidian / Excel / Postgres / Playwright / Slack / Linear / Figma 等）：[`resources/mcp-skills-catalog.md`](/zh-tw/resources/mcp-skills-catalog/)——81+ 個分類整理，每個都有 stars / license / 適合誰。下表只列「寫自己 MCP server / 找 reference」用的核心 catalog。

| 分類 | Project | ⭐ | 適合誰 | 為什麼推薦 / 備註 |
|---|---|---|---|---|
| **MCP server collection**<br>（接 CLI 用） | [modelcontextprotocol/servers](https://github.com/modelcontextprotocol/servers) | ⭐⭐⭐⭐⭐ | 第一個 MCP 從 reference 學起 | 官方 reference servers（現有 7 個：everything / fetch / filesystem / git / memory / sequentialthinking / time；github、sqlite 已移到 `servers-archived`），★ 89k+。詳見 [Stage 5.2](/zh-tw/stages/05-claude-code-ecosystem/#52--mcpmodel-context-protocol-基礎) |
...
| | [continuedev/continue](https://github.com/continuedev/continue) | ⭐⭐⭐⭐ | 想把 AI checks 接到 PR pipeline 強制執行 | ★ 35k+。完整介紹見 [`branches/for-developer.md`](/zh-tw/branches/for-developer/) |
| **Observability + Cost** | [langfuse/langfuse](https://github.com/langfuse/langfuse) | ⭐⭐⭐⭐⭐ | 想把 trace / cost / session 都接起來 | open source LLM observability，★ 32k+。詳見 [Stage 7 常用推薦](/zh-tw/stages/07-multi-agent-production/#-常用-multi-agent--production-工具推薦按用途分類) |
| | [Helicone](https://github.com/Helicone/helicone) | ⭐⭐⭐⭐ | 想要最快的 logging（改 base_url 就好）| proxy-based 監控、改 base_url 就有 logging + caching，★ 6k+ |
| | [promptfoo/promptfoo](https://github.com/promptfoo/promptfoo) | ⭐⭐⭐⭐⭐ | CLI workflow 升 production 前跑回歸測試 | eval framework，★ 24k+。詳見 [Stage 7 Agent Benchmark Landscape + Reward-Hacking 警告](/zh-tw/stages/07-multi-agent-production/#-agent-benchmark-landscape怎麼看不要只看排行榜---reward-hacking-警告) |
...
如果都可以 → **Track A 完整通關**。建議接著走 [**Stage 8 — Agent Interfaces**](/zh-tw/stages/08-agent-interfaces/)（**兩 track 共用 hub**：Computer Use / Browser Use / Code Sandbox，Track A 視角約 1-2 週），或挑一個 [specialized branch](/zh-tw#-學習地圖兩條學習路徑) 繼續走（researcher / developer / teacher / knowledge-worker / everyday-users）。
...
1. **加深 CLI workflow**（持續優化你的 setup）
   ...
   - 每季 review 一次 [`resources/cli-agents-guide.md`](/zh-tw/resources/cli-agents-guide/) 看新工具
