...
> 💡 **要找接日常工具的 MCP**（Notion / Obsidian / Excel / Postgres / Playwright / Slack / Linear / Figma 等）：[`resources/mcp-skills-catalog.md`](/zh-tw/resources/mcp-skills-catalog/)——65+ 個分類整理，每個都有 stars / license / 適合誰。下表只列「寫自己 MCP server / 找 reference」用的核心 catalog。
...
| **Observability + Cost** | [langfuse/langfuse](https://github.com/langfuse/langfuse) | ⭐⭐⭐⭐⭐ | 想把 trace / cost / session 都接起來 | open source LLM observability，★ 28k+。詳見 [Stage 7 常用推薦](/zh-tw/stages/07-multi-agent-production/#-常用-multi-agent--production-工具推薦按用途分類) |
...
| | [promptfoo/promptfoo](https://github.com/promptfoo/promptfoo) | ⭐⭐⭐⭐⭐ | CLI workflow 升 production 前跑回歸測試 | eval framework，★ 22k+。詳見 [Stage 7 Agent Benchmark Landscape + Reward-Hacking 警告](/zh-tw/stages/07-multi-agent-production/#-agent-benchmark-landscape怎麼看不要只看排行榜---reward-hacking-警告) |
