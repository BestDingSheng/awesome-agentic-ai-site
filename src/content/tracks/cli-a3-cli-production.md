---
title: "A3 — 把 CLI agent 接進安全的團隊流程"
description: "這一站只做一件事：讓 CLI agent 在測試用 PR 做一個只讀檢查。它可以提出意見，但不能自己合併、部署或取得多餘權限。"
section: "Track"
...
4. 需要 trace、eval 或完整 production 理論時，再進 [Stage 7](/zh-tw/stages/07-multi-agent-production/) 與 [Stage 7.5](/zh-tw/stages/07-5-advanced-agentic-concepts/)。
...
選 Anthropic 的 [`claude-code-action`](https://github.com/anthropics/claude-code-action) 或 OpenAI 的 [`codex-action`](https://github.com/openai/codex-action)。第一輪只在自己控制的 demo repo 與 branch 執行，沿用 A2 的 [`review-changes` Skill](/zh-tw/tracks/cli-a2-cli-workflow/#cli-6)。
...
其餘情況只記一個動作，理論放在 [Stage 7.5](/zh-tw/stages/07-5-advanced-agentic-concepts/)：
...
延伸閱讀：[`resources/subagent-cookbook.md`](/zh-tw/resources/subagent-cookbook/)與 [Stage 5.5](/zh-tw/stages/05-claude-code-ecosystem/#55--subagentsclaude-code-原生-multi-agent-機制-2025-新功能)。這些頁面之後會在自己的 layer 重新查證；使用 agent 名稱前仍以你當下的官方文件與實際清單為準。
</details>
