---
title: "A2 — 建立可重複使用的 CLI 工作流程（CLI Workflow Patterns）"
description: "裝好 CLI、跑過第一個任務之後，下一個問題：怎麼讓 CLI 一致地、可重複地、可分享地做事？這節講 workflow pattern——把「我每次都要重打一遍 prompt」變成「設好一次後 CLI 自己會用對方法」。"
section: "Track"
...
> 🔑 **關鍵名詞**：見 [`resources/glossary.md` 5](/zh-tw/resources/glossary/#5-claude-code-生態)（CLAUDE.md / slash command / SKILL.md / plugin / portable prompt）
...
- 完成 [A1](/zh-tw/tracks/cli-a1-cli-intro/)：選定主用 CLI、裝好、認證好、跑過至少 5 個非 hello-world 任務
- 寫過 1 份 `CLAUDE.md` / `AGENTS.md` / `GEMINI.md`（即使只是試水溫）
- 對 Stage 2 prompt engineering 基礎上手

沒到的話 → 先回 [A1](/zh-tw/tracks/cli-a1-cli-intro/) 把 CLI-1/2 練熟。
...
> ⭐ **進階補充：Claude Code 原生 multi-agent 機制**（這 1 句先看就好、不展開）：CLI-7 教的「手動拆 sub-task」其實 Claude Code 有 **Subagent / Agent team / Background agent** 三種原生工具可以自動化。完整 3 種機制 + 動手練習 + 何時不該用（團隊權限、上下文隔離、結果審查流程都要先想好）見 **[Stage 5.5](/zh-tw/stages/05-claude-code-ecosystem/#55--subagentsclaude-code-原生-multi-agent-機制-2025-新功能)**——在 A2 階段先知道有這層、不需要學細節。
...
|---|---|---|---|---|
| **CLAUDE.md 範例庫** | [Anthropic 官方 CLAUDE.md 指南](https://code.claude.com/docs/en/memory) | ⭐⭐⭐⭐⭐ | 第一份 CLAUDE.md 從這抄結構 | Claude Code repo 自己的 CLAUDE.md、官方寫法 |
| | [obra/superpowers](https://github.com/obra/superpowers) | ⭐⭐⭐⭐ | 看實際在用的 `.claude/` 完整目錄結構 | 不只是 skill collection、也是 CLAUDE.md 範本（★ 265k+） |
| | [mattpocock/skills](https://github.com/mattpocock/skills) | ⭐⭐⭐⭐ | 想看工程師日常用的 skill 庫 | `.claude/` 結構是好參考。**更多 skill 範例見 [Stage 5.3](/zh-tw/stages/05-claude-code-ecosystem/#53--skillsclaude-code-的行為層-claude-code-生態最關鍵的一層)** |
| **Slash Commands / Custom Prompts** | [anthropics/claude-plugins-official](https://github.com/anthropics/claude-plugins-official) | ⭐⭐⭐⭐⭐ | 找官方 plugin 範本 | 官方 plugin marketplace、每個 plugin 內的 commands / skills 是 slash command 範例（★ 32k+） |
...
| **Prompt 設計參考** | [f/awesome-chatgpt-prompts](https://github.com/f/awesome-chatgpt-prompts) | ⭐⭐⭐⭐ | 卡關時找 CLI 通用的 prompt 模式 | ChatGPT 起家、prompt 寫法 90% 在 CLI 上也通（★ 166k+、CC0）。完整 prompt engineering 進階見 [Stage 2 精選 Projects](/zh-tw/stages/02-prompt-engineering/#-精選-projects)（DSPy、dair-ai 等） |
| **多 CLI 並用 pattern** | [`resources/cli-agents-guide.md`](/zh-tw/resources/cli-agents-guide/) 「3 個常見搭配」 | ⭐⭐⭐⭐ | 想試多 CLI 配對策略 | 本 repo 內部資源、看 Setup A / B / C 挑一個合的試 |
...
- [**yamadashy/repomix**](https://github.com/yamadashy/repomix) ⭐⭐⭐⭐⭐ ★ 27k+ — 把整個 codebase packed 成單一 AI-friendly 檔案（XML / Markdown / JSON），方便 Claude Code / Codex 做 code review / refactoring。內建 MCP server mode + tree-sitter 壓縮（壓縮率依語言與檔案結構而異）+ secretlint 過濾敏感資訊。**Track A 很值得當 daily driver 的工具。**
- [**langchain-ai/openwiki**](https://github.com/langchain-ai/openwiki) ⭐⭐⭐⭐ ★ 14k+ — CLI，自動幫你的 codebase 生成並持續維護一份 wiki，並在 `CLAUDE.md` / `AGENTS.md` 裡加一條指向 wiki 的參照，讓 coding agent 需要時自己去讀、隨程式碼變動自動更新。`npm i -g openwiki` → `openwiki --init`。底層是 DeepAgents、可接 LangSmith 追蹤。MIT。

> 💡 **概念：*agent-facing documentation*（給 agent 讀的文件）。** repomix 跟 OpenWiki 在解同一個痛點（agent 不了解你的 repo），只是切角不同：一個是一次性打包快照，一個是會持續長大、自動維護的 wiki。共同的做法是給 agent *一份它需要時自己去讀的結構化 codebase context*，跟 `CLAUDE.md` 的指令分開放，而不是全部塞進 prompt。
