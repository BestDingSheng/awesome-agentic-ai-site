---
title: "术语小词典（Glossary）"
description: "本表是项目内强制统一的命名约定——所有 stage 用同一个中文理解名。如果你在 stage 内看到不一致，请报 issue。"
section: "Resource"
...
> [繁體中文](/zh-tw/resources/glossary/) | **简体中文** | [English](/en/resources/glossary/)
...
📍 详细：[Stage 1](/stages/01-llm-basics/)
...
📍 五种身份对照：[Track A A1](/tracks/cli-a1-cli-intro/)
...
📍 详细：[Stage 2](/stages/02-prompt-engineering/)
...
📍 详细：[Stage 3](/stages/03-tool-use-and-hello-agent/)
...
📍 详细：[Stage 3](/stages/03-tool-use-and-hello-agent/)
📍 schema 怎么写好：[Function Schema 设计 cheatsheet](/resources/schema-design-cheatsheet/)
...
⚠️ **这是 runner 里真的会跑的机械循环**：模型回答、调用工具或 Handoff、读回结果，再决定下一步。[Loop Engineering（循环工程）](#loop-engineering循环工程)则是设计这个循环和外围规则的工程工作；两者不是互斥的东西。完整分界见 [Stage 7](/stages/07-multi-agent-production/)。
...
📍 详细 + 路由：[Stage 3 反思](/stages/03-tool-use-and-hello-agent/#-反思reflexion--self-refine-概念--路由)
...
📍 详细：[Stage 6 Memory 是什么 + 如何设计](/stages/06-memory-rag/#-memory-是什么--怎么设计) + [Stage 6 CoALA Framework](/stages/06-memory-rag/#进阶coala-framework--agent-memory-的-4-层分类法)
...
📍 详细：[Stage 6](/stages/06-memory-rag/)
...
📍 详细：[Stage 6 进阶：带持久记忆的 Reflexion 完整版](/stages/06-memory-rag/#-进阶带持久记忆的-reflexion-完整版--track-b-选读)
...
📍 详细：[Stage 7](/stages/07-multi-agent-production/)
...
📍 详细：[Stage 5.2](/stages/05-claude-code-ecosystem/#52--mcpmodel-context-protocol-基础)
...
📍 入门：[Track A A2](/tracks/cli-a2-cli-workflow/)
...
📍 入门：[Track A A2](/tracks/cli-a2-cli-workflow/)；Claude Code 深入：[Stage 5.3](/stages/05-claude-code-ecosystem/#53--skillsclaude-code-的行为层-claude-code-生态最关键的一层)

### One-off Prompt（单次提示）

只为眼前任务使用的一次性交代，像今天才要用的便条。它放本次任务、范围、输入、禁止事项和成功条件；每次都相同的项目规则应移到 project instructions，重复流程应移到 Skill。

📍 练习：[Track A A2 CLI-8](/tracks/cli-a2-cli-workflow/#cli-8)

### Plugin / Marketplace

把多个 Skills + slash commands + hooks + MCP 设置打包成一个发布单位。**Marketplace** 就是 plugin 的目录，社群可以 `claude plugin install` 安装别人写好的。

📍 详细：[Stage 5.4](/stages/05-claude-code-ecosystem/#54--plugins-与-marketplaces)
...
📍 练习：[Track A A3 CLI-10](/tracks/cli-a3-cli-production/#cli-10)
...
📍 入门：[Stage 2](/stages/02-prompt-engineering/)；完整 eval harness：[Stage 7](/stages/07-multi-agent-production/)

### Observability

把 agent 执行过的步骤、模型、tool、usage、时间和结果留下来，像收据加行车记录。出 bug 时可以找出哪一步失败；拿不到的字段要写“未确认”，不能靠猜。常见工具有 Langfuse、Phoenix、Helicone。

📍 入门：[Track A A3 CLI-11](/tracks/cli-a3-cli-production/#cli-11)；深入：[Stage 7](/stages/07-multi-agent-production/)
...
📍 详细：[Track A A1](/tracks/cli-a1-cli-intro/)、[`resources/cli-agents-guide.zh-Hans.md`](/resources/cli-agents-guide/)
...
📍 详细：[Stage 2 结尾](/stages/02-prompt-engineering/) / [Stage 6](/stages/06-memory-rag/) / [Stage 7](/stages/07-multi-agent-production/)
...
📍 学科级概念（**8 个核心元件** / Prompt→Context→Harness→Loop→Graph 五层工程分工 / framework vs harness）：[Stage 7 Harness Engineering](/stages/07-multi-agent-production/)
📍 Reference implementation case study（读 Claude Code source）：[Stage 5 5.7](/stages/05-claude-code-ecosystem/)
...
这是正在形成的名称，不是本项目自创，也不是所有供应商共同制定的标准。入门来源：[IBM — Loop Engineering](https://www.ibm.com/think/topics/loop-engineering)；采用状态可看 [2026-08 exploratory preprint](https://arxiv.org/abs/2608.21884)。完整五层与实践入口见 [Stage 7](/stages/07-multi-agent-production/)。
...
- **这里的“图”是执行流程图（control / execution graph），不是 GraphRAG 那种知识图谱检索**——后者见 [Stage 6](/stages/06-memory-rag/)。两者常被混为一谈。
- **名称新，底下的做法不新。** 2026-08 的 survey preprint 把 Graph Engineering 提为新兴范式；workflow、state machine、node、edge 与 checkpoint 更早就存在。主流 SDK 目前仍常写 **workflow**、**graph-based workflow** 或 **orchestration**。

可搜索的实现名称与来源：[LangGraph — Workflows and agents](https://docs.langchain.com/oss/python/langgraph/workflows-agents)、[Microsoft Agent Framework — Workflow concepts](https://learn.microsoft.com/en-us/agent-framework/concepts/workflows/)、[Graph Engineering survey preprint](https://arxiv.org/abs/2608.21156)。Preprint 证明这个总称正在形成，不代表它已成为官方标准。

**跟循环的关系**：不是二选一。**格子里面是 agent 自己绕圈，格子跟格子之间才是你安排的顺序**——所以图是把好几个循环装进格子再排顺序；全部塞回同一个格子，就退回单纯的循环了。格子里也不一定是 agent，可以是一个工具、一段检查、或“这里要人按核准才能往下”。五层阶梯的完整说明见 [Stage 7](/stages/07-multi-agent-production/)（canonical）。

先在 [Stage 4 的 Agent framework／Workflow Graph](/stages/04-agent-frameworks/) 学工具和基本图，再到 [Stage 7](/stages/07-multi-agent-production/) 加上预算、验证、观测与恢复。可直接运行的入口是 [`examples/stage-4/03-graph-workflow/`](/)（`StateGraph` / conditional edge / checkpointer）。相关：harness、Loop Engineering、orchestration。
...
📍 完整 loop、现行工具和 benchmark 读法：[Stage 8 Computer Use](/stages/08-agent-interfaces/#-computer-use--屏幕级智能体)
...
📍 完整信号比较与现行框架：[Stage 8 Browser Use](/stages/08-agent-interfaces/#-browser-use--web-级智能体)
...
📍 完整 9-row 术语小词典和 provider 选择：[Stage 8 Code Sandbox](/stages/08-agent-interfaces/#-code-execution-sandbox--隔离环境含术语小词典)
...
📍 完整对比：[Stage 8 术语小词典](/stages/08-agent-interfaces/#-隔离技术术语小词典)

### Firecracker

AWS 开源、用 Rust 编写的 microVM 技术。它提供建立轻量 VM 的底层能力；真正的 sandbox 还要再加上网络、文件、秘密、生命周期和审计策略。

📍 [Stage 8 术语小词典](/stages/08-agent-interfaces/#-隔离技术术语小词典)
...
- 看 [Stage 5.2 — MCP](/stages/05-claude-code-ecosystem/#52--mcpmodel-context-protocol-基础) / [5.3 — Skills](/stages/05-claude-code-ecosystem/#53--skillsclaude-code-的行为层-claude-code-生态最关键的一层) / [5.4 — Plugins](/stages/05-claude-code-ecosystem/#54--plugins-与-marketplaces) 的内文
- 看 [Stage 1](/stages/01-llm-basics/) / [Stage 6](/stages/06-memory-rag/) / [Stage 7](/stages/07-multi-agent-production/) / [Stage 8](/stages/08-agent-interfaces/) 的延伸阅读清单
