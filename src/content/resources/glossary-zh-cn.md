...
⚠️ **这是“一次执行里面”的循环**，是 harness 的一个零件，跟五层阶梯第 4 层的 [Loop Engineering（循环工程）](#loop-engineering循环工程)（管的是跨 session 的长时间执行）同名但不同层次。两者的分界见 [Stage 7](/stages/07-multi-agent-production/)。
...
📍 学科级概念（**8 个核心元件** / prompt→context→harness 五层工程分工 / framework vs harness）：[Stage 7 Harness Engineering](/stages/07-multi-agent-production/)
...
五层工程分工的第 4 层（完整阶梯与各层目的见 [Stage 7](/stages/07-multi-agent-production/)，那里是 canonical）：设计 / 调校 agent 的“迭代循环”本身——目标、工具、context 管理、终止条件、错误处理，让长时间（数百步、跨 session）运行仍可靠、可控、不跑偏。相关：harness、Dynamic Workflows、ReAct。
...
真正要学的东西在 [Stage 4 的 multi-agent pattern](/stages/04-agent-frameworks/) 和可以直接跑的 [`examples/stage-4/03-graph-workflow/`](/)（`StateGraph` / conditional edge / checkpointer）。相关：harness、Loop Engineering、orchestration。
...
📍 完整解说 + 4 强对比：[Stage 8 Computer Use](/stages/08-agent-interfaces/)
...
📍 完整解说 + 5 强对比 + OSS 框架：[Stage 8 Browser Use](/stages/08-agent-interfaces/)
...
📍 完整 9-row 术语小词典（含 microVM / Container 差异）+ 7 强对比：[Stage 8 Code Sandbox](/stages/08-agent-interfaces/)
...
📍 完整对比：[Stage 8 术语小词典](/stages/08-agent-interfaces/)

### Firecracker

AWS 开源的 microVM、Rust 写、**AWS Lambda 底层** + E2B sandbox 用它做 isolation。强隔离 + 快启动兼顾。

📍 [Stage 8 术语小词典](/stages/08-agent-interfaces/)
