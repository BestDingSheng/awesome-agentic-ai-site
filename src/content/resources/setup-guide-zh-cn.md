...
description: "这一页不是要你把所有工具都装一遍。你只要先选一扇门，完成一个小结果。"
section: "Resource"
...
已经会用 Python、Git 和 terminal，也知道怎么保护 **API Key（API 密钥）**？可以直接去 [Stage 1](/stages/01-llm-basics/)。
...
| 写 code 时让 AI 在旁边协助 | **IDE Assistant**：住在 editor 里 | 先看[开发者路线](/branches/for-developer/) |
| 让 Agent 在指定文件夹读文件、改文件、跑命令 | **CLI Agent**：在 terminal 里工作 | 先看 [CLI Agents 指南](/resources/cli-agents-guide/) |
...
本地推理没有供应商模型 API 账单，但硬件、电力、下载时间、设备安全、文件和 log 仍由你负责。完整路径请看 [Cookbook 的本地 LLM walkthrough](/resources/cookbook/#6-本地-llm--cli-agent-快速-walkthrough)。
...
更完整的责任边界请看 [Stage 5.3 — Skills](/stages/05-claude-code-ecosystem/#53--skills按需操作卡)，更多示例请看 [Cookbook](/resources/cookbook/)。
...
| 你现在想做什么 | 下一站 |
|---|---|
| 理解模型、Token、Context Window 与 API | [Stage 1 — LLM 基础](/stages/01-llm-basics/) |
| 直接学 Prompt | [Stage 2 — Prompt Engineering](/stages/02-prompt-engineering/) |
| 使用 CLI Agent 工作 | [Track A1 — CLI 入门](/tracks/cli-a1-cli-intro/) |
| 理解 Claude Code、MCP、Skills、Plugins 与 Subagents | [Stage 5 — Claude Code 生态](/stages/05-claude-code-ecosystem/) |
| 使用本地模型 | [Cookbook：本地 LLM walkthrough](/resources/cookbook/#6-本地-llm--cli-agent-快速-walkthrough) |
| 还分不清 OpenRouter、Ollama、OpenCode 或 Pi | [Glossary：先分清五种工具身份](/resources/glossary/#-先分清五种工具身份) |
