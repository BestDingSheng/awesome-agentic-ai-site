---
title: "Subagent Cookbook — 15 个复制粘贴就能用的派遣 recipe"
description: "| 段 | 内容 | 为什么有这段 | |---|---|---| | 场景 | 你今天工作会遇到的具体场景 | 从“我有 X 问题”找到 recipe，而不是从“我想用 subagent”找 | | Subagent | 用哪一个（Claude Code 内置名称） | 直接 copy 名字，不用想 | | Prompt 模板 | 复制粘贴即可用的指令文字 | 不用自己想怎么写 | | 何时不用 | 比用 subagent 更好的替代"
section: "Resource"
...
> [繁體中文](/zh-tw/resources/subagent-cookbook/) | **简体中文** | [English](/en/resources/subagent-cookbook/)

> 📋 **这是什么**：[Stage 5.5](/stages/05-claude-code-ecosystem/#55--subagentsclaude-code-原生-multi-agent-机制-2025-新功能) 教你 subagent 是什么概念，这份 cookbook 教你**今天就能用**——15 个场景，每个都包含“该用哪个 subagent + 完整 prompt 模板（复制即可用）+ 何时不用”。
>
...
- **想理解完整理论**（subagent 跟 skill / MCP 的差别、3 种 multi-agent 机制）→ [Stage 5.5](/stages/05-claude-code-ecosystem/#55--subagentsclaude-code-原生-multi-agent-机制-2025-新功能)
- **CLI 日常用法 playbook** → [`tracks/cli/A3-cli-production.zh-Hans.md` Playbook 4](/tracks/cli-a3-cli-production/#-playbook-4派遣-subagent-跑独立任务)
- **想看 subagent 在 agent paradigm 体系内的定位** → [`resources/agent-paradigms.zh-Hans.md`](/resources/agent-paradigms/#subagent--在-agent-runtime-里再-spawn-agent)
- **词汇快查** → [`resources/glossary.zh-Hans.md` § 5. Claude Code 生态 — Subagent](/resources/glossary/#subagent子-agent)
