...
description: "看到陌生英文時，不用停下整章。先在這裡找到一句白話解釋，再回原本的 Stage 繼續做。"
section: "Resource"
...
**Context Window** 是模型這一次能一起參考的 token 空間。空間大不代表每段資料都會被同樣注意；先放任務真正需要的內容，再到 [Stage 1](/zh-tw/stages/01-llm-basics/) 查目前型號的正式上限。
...
📍 選修導覽：[模型訓練與調整指南](/zh-tw/resources/model-training-guide/)
...
📍 詳細：[Stage 4](/zh-tw/stages/04-agent-frameworks/)
...
📍 教學：[Stage 5.5](/zh-tw/stages/05-claude-code-ecosystem/#55--subagentsclaude-code-原生-multi-agent-機制-2025-新功能) · [可複製 recipes](/zh-tw/resources/subagent-cookbook/) · [進階組合](/zh-tw/resources/subagent-advanced/)
...
📍 入門：[Stage 2](/zh-tw/stages/02-prompt-engineering/)；Agent 系統：[Stage 7](/zh-tw/stages/07-multi-agent-production/)
...
**Frontier Model** 是某個時間點能力位於前沿的模型類別，不是一個永久名單。型號、價格、Context 與可用狀態變動很快；目前資料統一看 [Stage 1 的官方來源表](/zh-tw/stages/01-llm-basics/)。
...
這裡的 graph 是執行流程，不是 Stage 6 的 GraphRAG 知識圖譜。先在 [Stage 4](/zh-tw/stages/04-agent-frameworks/)學基本 Workflow Graph，再到 [Stage 7](/zh-tw/stages/07-multi-agent-production/)加入上線邊界。
...
要比較 Search／Fetch、Browser Use、Computer Use 與 Sandbox，回到 [Stage 8](/zh-tw/stages/08-agent-interfaces/)。
...
- 看 [Stage 5.2 的 MCP](/zh-tw/stages/05-claude-code-ecosystem/#52--mcpmodel-context-protocol-基礎)、[Stage 5.3 的 Skills](/zh-tw/stages/05-claude-code-ecosystem/#53--skillsclaude-code-的行為層-claude-code-生態最關鍵的一層)或 [Stage 7 的 production 邊界](/zh-tw/stages/07-multi-agent-production/)。
