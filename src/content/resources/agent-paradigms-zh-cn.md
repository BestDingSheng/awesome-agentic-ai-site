---
title: "Agent 工具怎么分：身份、操作界面、部署位置"
description: "同一个工具可以出现在终端、IDE 和桌面应用中，也可以连接本地或云端模型。所以不要硬把工具塞进五个互斥“类型”。先问三个问题，会比较不容易混乱。"
section: "Resource"
...
---
> [繁體中文](/zh-tw/resources/agent-paradigms/) | **简体中文** | [English](/en/resources/agent-paradigms/)
...
![Agent 工具的 Identity、Surface 和 Deployment 三条轴](/upstream/resources/diagrams/agent-tool-axes.zh-Hans.png)
...
1. [CLI Agents 指南](/resources/cli-agents-guide/)：比较登录、Provider、Sandbox、项目规则和权限。
2. [Stage 4：Workflow Graph 与 Agent 框架](/stages/04-agent-frameworks/)：学习 Framework 和 Workflow Graph。
3. [Stage 5：Claude Code 生态](/stages/05-claude-code-ecosystem/)：学习 Skills、MCP、Hooks 和 Subagents。
4. [Stage 7：Agent Production Engineering](/stages/07-multi-agent-production/)：学习 Harness、Loop、Graph、跨系统 Eval 和上线边界。
...
继续阅读 [Stage 5 的 Subagents](/stages/05-claude-code-ecosystem/) 和[可直接复制的 Subagent Cookbook](/resources/subagent-cookbook/)。
