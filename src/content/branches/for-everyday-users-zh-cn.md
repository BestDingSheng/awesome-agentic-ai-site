...
description: "这条路教你把 AI 当成“先帮忙写草稿的助手”。你先给它材料和要求，它先做一版；最后仍由你对照原文、修正并决定要不要使用。"
section: "Branch"
...
想系统学习 Prompt、zero-shot、one-shot、few-shot 与查证方法，再进入 [Stage 2 — Prompt Engineering](/stages/02-prompt-engineering/)。
...
- 想把 Prompt 写得更清楚：进入 [Stage 2](/stages/02-prompt-engineering/)。
- 想安全使用 CLI Agent：进入 [Track A1](/tracks/cli-a1-cli-intro/)。
- 想分清 App、Connector、MCP 与自动化：进入 [知识工作者路线](/branches/for-knowledge-worker/)。
