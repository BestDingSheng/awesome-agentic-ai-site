...
description: "AI 程式助手像一位會讀檔案、改程式、跑指令的隊友。它做得快，也可能做錯。這條路教你先把任務縮小，再看懂每個改動，最後由人決定要不要留下。"
section: "Branch"
...
建議路線：`A1 → A2 → Stage 5 的 5.1–5.4 → A3`。可以從 [A1](/zh-tw/tracks/cli-a1-cli-intro/)、[A2](/zh-tw/tracks/cli-a2-cli-workflow/)、[Stage 5](/zh-tw/stages/05-claude-code-ecosystem/) 和 [A3](/zh-tw/tracks/cli-a3-cli-production/) 依序前進；[Stage 8](/zh-tw/stages/08-agent-interfaces/) 建議完成，但不擋你先開始這條路。已走 Track B 的讀者，可以先讀 [Stage 7](/zh-tw/stages/07-multi-agent-production/)。
...
下一站：要設計 Skills／MCP，走 [Stage 5](/zh-tw/stages/05-claude-code-ecosystem/)；要做 Eval、Observability 與 production gate，走 [Stage 7](/zh-tw/stages/07-multi-agent-production/)；要比較 CLI agents，打開 [CLI agent 指南](/zh-tw/resources/cli-agents-guide/)。
