...
> **繁體中文** | [简体中文](/branches/for-developer-zh-cn/) | [English](/branches/for-developer-en/)
...
> 6 個主流 CLI agent（Claude Code / Codex / OpenCode / Gemini CLI / goose / Aider）的並列比較見 [`resources/cli-agents-guide.md`](/resources/cli-agents-guide/)。第一次接觸 CLI agent 想要 step-by-step 入門 → [`tracks/cli/A1-cli-intro.md`](/tracks/a1-cli-intro/)（Track A 第一站）。要把 CLI 接到日常工具（GitHub、Linear、Atlassian、Postgres、Playwright、Figma 等）→ [`resources/mcp-skills-catalog.md`](/resources/mcp-skills-catalog/)（62 個分類整理）。下面只列開發者該知道的關鍵 entry。
...
- **Tier 3**：CI 自動跑 agent + production observability——進到 [Stage 7](/stages/07-multi-agent-production/) 領域
...
- **要做 ML 研究 / 寫 paper** → [研究員分支](/branches/for-researcher/)
- **接 Notion / Linear / Atlassian / Postgres / Figma** 等 dev tool → [`resources/mcp-skills-catalog.md`](/resources/mcp-skills-catalog/)
- **要寫自己的 Skill / MCP server** → [Stage 5](/stages/05-claude-code-ecosystem/) + [`resources/cookbook.md`](/resources/cookbook/)
- **想看 schema 設計細節** → [`resources/schema-design-cheatsheet.md`](/resources/schema-design-cheatsheet/)
- **CLI 從零開始** → [Track A](/tracks/a1-cli-intro/)（A1 → A2 → A3）
