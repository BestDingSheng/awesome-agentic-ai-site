...
description: "把散亂的會議紀錄、Email、文件與待辦，整理成「看得懂、找得到、有人負責」的工作成果。AI 可以幫你先整理；來源、權限與最後決定仍由人負責。"
section: "Branch"
...
> [← 回主路線](/zh-tw) · 走完 **Track A 的 A3** 或 **Track B 的 Stage 7** 後從這裡接續。沒有開發背景也沒關係：先做一次性任務，需要重複時才接工具。
...
不要因為看見 MCP 就先裝 MCP。先問：「現有服務內的 App／Connector 能不能安全完成？」只有需要自訂工具或跨 client 重用時，才往 [Stage 5.2 — MCP 基礎](/zh-tw/stages/05-claude-code-ecosystem/#52--mcpmodel-context-protocol-基礎)前進。
...
接下來：要做自訂連線，回到 [Stage 5.2 — MCP](/zh-tw/stages/05-claude-code-ecosystem/#52--mcpmodel-context-protocol-基礎)；要做長時間流程，前往 [Stage 7 — Loop／Graph Engineering](/zh-tw/stages/07-multi-agent-production/)；要自己寫或審查程式，走[開發者路線](/zh-tw/branches/for-developer/)。
