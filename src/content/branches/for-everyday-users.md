---
title: "日常使用者延伸路線（For Everyday Users）"
description: "這條路教你把 AI 當成「先幫忙寫草稿的助手」。你先給它資料和要求，它先做一版；最後仍由你對照原文、修正並決定要不要使用。"
section: "Branch"
...
> [← 回主路線](/zh-tw) · 你不必先學會寫程式，也不必走完整條主幹。
...
如果你只想聊天，不需要安裝 CLI Agent 或本地 Runtime。想學命令列時再去 [Track A 第一站](/zh-tw/tracks/cli-a1-cli-intro/)；想了解模型時再去 [Stage 1](/zh-tw/stages/01-llm-basics/)。
...
想系統學 Prompt、zero-shot、one-shot、few-shot 與查證方法，再進 [Stage 2 — Prompt Engineering](/zh-tw/stages/02-prompt-engineering/)。
...
- [OpenCode permissions](https://opencode.ai/docs/agents/) 可對 edit、bash 與外部資料夾設定 ask／allow／deny；[provider 文件](https://v2.opencode.ai/docs/providers/) 顯示模型連線仍需要對應帳號、OAuth、API key 或環境設定。
...
- 想把 Prompt 寫得更清楚：進 [Stage 2](/zh-tw/stages/02-prompt-engineering/)。
- 想安全使用 CLI Agent：進 [Track A1](/zh-tw/tracks/cli-a1-cli-intro/)。
- 想分清 App、Connector、MCP 與自動化：進 [知識工作者路線](/zh-tw/branches/for-knowledge-worker/)。
