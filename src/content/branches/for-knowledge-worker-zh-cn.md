...
description: "常见工作包括：Email 分流、会议转行动项目、每周报告、产品需求整理、研究摘要与知识库整理。"
section: "Branch"
...
不要因为看见 MCP 就先安装 MCP。先问：“现有服务里的 App／Connector 能不能安全完成？”只有需要自定义工具或跨 client 复用时，才进入 [Stage 5.2 — MCP 基础](/stages/05-claude-code-ecosystem/#52--mcpmodel-context-protocol-基础)。
...
下一步：自定义连接看 [Stage 5.2 — MCP](/stages/05-claude-code-ecosystem/#52--mcpmodel-context-protocol-基础)；长流程看 [Stage 7](/stages/07-multi-agent-production/)；写或审查代码走[开发者路线](/branches/for-developer/)。
